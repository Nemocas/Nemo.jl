...
function divexact!(a::ZZMatrix, b::ZZMatrix, d::ZZRingElem)
  ccall((:fmpz_mat_scalar_divexact_fmpz, libflint), Nothing, (Ref{ZZMatrix}, Ref{ZZMatrix}, Ref{ZZRingElem}), a, b, d)
end

function mul!(a::ZZMatrix, b::ZZMatrix, c::ZZRingElem)
  ccall((:fmpz_mat_scalar_mul_fmpz, libflint), Nothing, (Ref{ZZMatrix}, Ref{ZZMatrix}, Ref{ZZRingElem}), a, b, c)
end
...
function snf_with_transform(A::ZZMatrix, l::Bool=true, r::Bool=true)
  ...
  for i in 1:min(nrows(S), ncols(S))
    ...
    for j in (i + 1):min(nrows(S), ncols(S))
      if S[j, j] == 0
        continue
      end
...
function prod_diagonal(A::MatrixElem{T}) where {T}
  ...
  return prod(T[A[i, i] for i in 1:nrows(A)])
end
