...
function Base.view(x::QQMatrix, r1::Int, c1::Int, r2::Int, c2::Int)
  ...
  b = QQMatrix()
  b.view_parent = x
  ccall((:fmpq_mat_window_init, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}, Int, Int, Int, Int), b, x, r1 - 1, c1 - 1, r2, c2)
...
function getindex!(v::QQFieldElem, a::QQMatrix, r::Int, c::Int)
  GC.@preserve a begin
    z = ccall((:fmpq_mat_entry, libflint), Ptr{QQFieldElem}, (Ref{QQMatrix}, Int, Int), a, r - 1, c - 1)
...
@inline function setindex!(a::QQMatrix, d::ZZRingElem, r::Int, c::Int)
  @boundscheck Generic._checkbounds(a, r, c)
  GC.@preserve a begin
    z = ccall((:fmpq_mat_entry_num, libflint), Ptr{ZZRingElem}, (Ref{QQMatrix}, Int, Int), a, r - 1, c - 1)
    ccall((:fmpz_set, libflint), Nothing, (Ptr{ZZRingElem}, Ref{ZZRingElem}), z, d)
    z = ccall((:fmpq_mat_entry_den, libflint), Ptr{ZZRingElem}, (Ref{QQMatrix}, Int, Int), a, r - 1, c - 1)
...
Base.@propagate_inbounds setindex!(a::QQMatrix, d::Integer, r::Int, c::Int) = setindex!(a, QQFieldElem(d), r, c)
...
function setindex!(a::QQMatrix, b::QQMatrix, r::UnitRange{Int64}, c::UnitRange{Int64})
  ...
  A = view(a, r, c)
  ccall((:fmpq_mat_set, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}), A, b)
end

Base.@propagate_inbounds setindex!(a::QQMatrix, d::Rational, r::Int, c::Int) = setindex!(a, QQFieldElem(d), r, c)

number_of_rows(a::QQMatrix) = a.r

number_of_columns(a::QQMatrix) = a.c

iszero(a::QQMatrix) = ccall((:fmpq_mat_is_zero, libflint), Bool, (Ref{QQMatrix},), a)
...
isone(a::QQMatrix) = ccall((:fmpq_mat_is_one, libflint), Bool, (Ref{QQMatrix},), a)
...
function -(x::QQMatrix)
  z = similar(x)
  ccall((:fmpq_mat_neg, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}), z, x)
  return z
end
...
function transpose(x::QQMatrix)
  ...
  ccall((:fmpq_mat_transpose, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}), z, x)
  return z
end

function transpose!(A::QQMatrix, B::QQMatrix)
  ccall((:fmpq_mat_transpose, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}), A, B)
  return A
end
...
function swap_rows!(x::QQMatrix, i::Int, j::Int)
  ccall((:fmpq_mat_swap_rows, libflint), Nothing, (Ref{QQMatrix}, Ptr{Nothing}, Int, Int), x, C_NULL, i - 1, j - 1)
  return x
end
...
function swap_cols!(x::QQMatrix, i::Int, j::Int)
  ccall((:fmpq_mat_swap_cols, libflint), Nothing, (Ref{QQMatrix}, Ptr{Nothing}, Int, Int), x, C_NULL, i - 1, j - 1)
  return x
end
...
function reverse_rows!(x::QQMatrix)
  ccall((:fmpq_mat_invert_rows, libflint), Nothing, (Ref{QQMatrix}, Ptr{Nothing}), x, C_NULL)
  return x
end
...
function reverse_cols!(x::QQMatrix)
  ccall((:fmpq_mat_invert_cols, libflint), Nothing, (Ref{QQMatrix}, Ptr{Nothing}), x, C_NULL)
  return x
end
...
function +(x::QQMatrix, y::QQMatrix)
  check_parent(x, y)
  z = similar(x)
  ccall((:fmpq_mat_add, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{QQMatrix}), z, x, y)
  return z
end
...
function -(x::QQMatrix, y::QQMatrix)
  check_parent(x, y)
  z = similar(x)
  ccall((:fmpq_mat_sub, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{QQMatrix}), z, x, y)
  return z
end
...
function *(x::QQMatrix, y::QQMatrix)
  ...
  ccall((:fmpq_mat_mul, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{QQMatrix}), z, x, y)
  return z
end
...
function *(x::ZZRingElem, y::QQMatrix)
  z = similar(y)
  ccall((:fmpq_mat_scalar_mul_fmpz, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{ZZRingElem}), z, y, x)
  return z
end

function *(x::QQFieldElem, y::QQMatrix)
  z = similar(y)
  ccall((:fmpq_mat_scalar_mul_fmpz, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{QQFieldElem}), z, y, numerator(x))
  ccall((:fmpq_mat_scalar_div_fmpz, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{QQFieldElem}), z, z, denominator(x))
  return z
end
...
function ==(x::QQMatrix, y::QQMatrix)
  ...
  fl && ccall((:fmpq_mat_equal, libflint), Bool, (Ref{QQMatrix}, Ref{QQMatrix}), x, y)
end
...
function inv(x::QQMatrix)
  ...
  z = similar(x)
  success = ccall((:fmpq_mat_inv, libflint), Cint, (Ref{QQMatrix}, Ref{QQMatrix}), z, x)
...
function divexact(x::QQMatrix, y::QQFieldElem; check::Bool=true)
  z = similar(x)
  ccall((:fmpq_mat_scalar_div_fmpz, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{ZZRingElem}), z, x, numerator(y))
  ccall((:fmpq_mat_scalar_mul_fmpz, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{ZZRingElem}), z, z, denominator(y))
  return z
end

function divexact(x::QQMatrix, y::ZZRingElem; check::Bool=true)
  z = similar(x)
  ccall((:fmpq_mat_scalar_div_fmpz, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{ZZRingElem}), z, x, y)
  return z
end
...
function kronecker_product(x::QQMatrix, y::QQMatrix)
  ...
  ccall((:fmpq_mat_kronecker_product, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{QQMatrix}), z, x, y)
  return z
end
...
function charpoly(R::QQPolyRing, x::QQMatrix)
  ...
  z = R()
  ccall((:fmpq_mat_charpoly, libflint), Nothing, (Ref{QQPolyRingElem}, Ref{QQMatrix}), z, x)
  return z
end
...
function minpoly(R::QQPolyRing, x::QQMatrix)
  nrows(x) != ncols(x) && error("Non-square")
  z = R()
  ccall((:fmpq_mat_minpoly, libflint), Nothing, (Ref{QQPolyRingElem}, Ref{QQMatrix}), z, x)
  return z
end
...
function det(x::QQMatrix)
  ...
  z = QQFieldElem()
  ccall((:fmpq_mat_det, libflint), Nothing, (Ref{QQFieldElem}, Ref{QQMatrix}), z, x)
  return z
end
...
function gram_schmidt_orthogonalisation(x::QQMatrix)
  z = similar(x)
  ccall((:fmpq_mat_gso, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}), z, x)
  return z
end
...
function hilbert(R::QQMatrixSpace)
  z = R()
  ccall((:fmpq_mat_hilbert_matrix, libflint), Bool, (Ref{QQMatrix},), z)
  return z
end
...
function rank(x::QQMatrix)
  z = similar(x)
  r = ccall((:fmpq_mat_rref, libflint), Int, (Ref{QQMatrix}, Ref{QQMatrix}), z, x)
  return r
end
...
function rref!(x::QQMatrix)
  r = ccall((:fmpq_mat_rref, libflint), Int, (Ref{QQMatrix}, Ref{QQMatrix}), x, x)
  return r
end
...
function _solve_dixon(a::QQMatrix, b::QQMatrix)
  ...
  z = similar(b)
  nonsing = ccall((:fmpq_mat_solve_dixon, libflint), Bool, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{QQMatrix}), z, a, b)
...
function Solve._can_solve_internal_no_check(A::QQMatrix, b::QQMatrix, task::Symbol; side::Symbol=:left)
  ...
  fl = ccall((:fmpq_mat_can_solve_multi_mod, libflint), Cint, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{QQMatrix}), x, A, b)
...
function tr(x::QQMatrix)
  ...
  d = QQFieldElem()
  ccall((:fmpq_mat_trace, libflint), Nothing, (Ref{QQFieldElem}, Ref{QQMatrix}), d, x)
  return d
end
...
function hcat(a::QQMatrix, b::QQMatrix)
  ...
  ccall((:fmpq_mat_concat_horizontal, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{QQMatrix}), c, a, b)
  return c
end
...
function vcat(a::QQMatrix, b::QQMatrix)
  ...
  ccall((:fmpq_mat_concat_vertical, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{QQMatrix}), c, a, b)
  return c
end
...
function similarity!(z::QQMatrix, r::Int, d::QQFieldElem)
  ccall((:fmpq_mat_similarity, libflint), Nothing, (Ref{QQMatrix}, Int, Ref{QQFieldElem}), z, r - 1, d)
end
...
function mul!(z::Vector{QQFieldElem}, a::Vector{QQFieldElem}, b::QQMatrix)
  ccall((:fmpq_mat_fmpq_vec_mul_ptr, libflint), Nothing, (Ptr{Ref{QQFieldElem}}, Ptr{Ref{QQFieldElem}}, Int, Ref{QQMatrix}), z, a, length(a), b)
  return z
end

function mul!(z::Vector{QQFieldElem}, a::QQMatrix, b::Vector{QQFieldElem})
  ccall((:fmpq_mat_mul_fmpq_vec_ptr, libflint), Nothing, (Ptr{Ref{QQFieldElem}}, Ref{QQMatrix}, Ptr{Ref{QQFieldElem}}, Int), z, a, b, length(b))
  return z
end

function mul!(z::Vector{QQFieldElem}, a::Vector{ZZRingElem}, b::QQMatrix)
  ccall((:fmpq_mat_fmpz_vec_mul_ptr, libflint), Nothing, (Ptr{Ref{QQFieldElem}}, Ptr{Ref{ZZRingElem}}, Int, Ref{QQMatrix}), z, a, length(a), b)
  return z
end

function mul!(z::Vector{QQFieldElem}, a::QQMatrix, b::Vector{ZZRingElem})
  ccall((:fmpq_mat_mul_fmpz_vec_ptr, libflint), Nothing, (Ptr{Ref{QQFieldElem}}, Ref{QQMatrix}, Ptr{Ref{ZZRingElem}}, Int), z, a, b, length(b))
  return z
end
...
function mul!(y::QQMatrix, x::Int)
  ccall((:fmpq_mat_scalar_mul_fmpz, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{QQFieldElem}), y, y, ZZRingElem(x))
  return y
end

function mul!(y::QQMatrix, x::ZZRingElem)
  ccall((:fmpq_mat_scalar_mul_fmpz, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{ZZRingElem}), y, y, x)
  return y
end

function addeq!(z::QQMatrix, x::QQMatrix)
  ccall((:fmpq_mat_add, libflint), Nothing, (Ref{QQMatrix}, Ref{QQMatrix}, Ref{QQMatrix}), z, z, x)
  return z
end

function zero!(z::QQMatrix)
  ccall((:fmpq_mat_zero, libflint), Nothing, (Ref{QQMatrix},), z)
  return z
end
...
function Generic.add_one!(a::QQMatrix, i::Int, j::Int)
  ...
  GC.@preserve a begin
    x = mat_entry_ptr(a, i, j)
    ccall((:fmpq_add_si, libflint), Nothing, (Ptr{QQFieldElem}, Ptr{QQFieldElem}, Int), x, x, 1)
  end
  return a
end
...
@inline mat_entry_ptr(A::QQMatrix, i::Int, j::Int) = ccall((:fmpq_mat_entry, libflint), Ptr{QQFieldElem}, (Ref{QQMatrix}, Int, Int), A, i - 1, j - 1)
...
function nullspace(A::QQMatrix)
  ...
  ccall((:fmpq_mat_get_fmpz_mat_rowwise, libflint), Nothing, (Ref{ZZMatrix}, Ptr{Nothing}, Ref{QQMatrix}), AZZ, C_NULL, A)
  N = similar(AZZ, ncols(A), ncols(A))
  nullity = ccall((:fmpz_mat_nullspace, libflint), Int, (Ref{ZZMatrix}, Ref{ZZMatrix}), N, AZZ)
  NQQ = similar(A, ncols(A), ncols(A))
  ccall((:fmpq_mat_set_fmpz_mat, libflint), Nothing, (Ref{QQMatrix}, Ref{ZZMatrix}), NQQ, N)
