...
for (etype, rtype, ctype, btype, flint_fn, flint_tail) in (
  ...
  @eval begin
    ...
    function is_gen(a::($etype))
      return precision(a) == 0 || ccall(($(flint_fn * "_is_gen"), libflint), Bool, (Ref{($etype)}, Ref{($ctype)}), a, base_ring(a))
    end
    ...
    function isone(a::($etype))
      return precision(a) == 0 || ccall(($(flint_fn * "_is_one"), libflint), Bool, (Ref{($etype)}, Ref{($ctype)}), a, base_ring(a))
    end
    ...
    function similar(f::AbsPowerSeriesRingElem, R::($ctype), max_prec::Int, s::Symbol=var(parent(f)); cached::Bool=true)
    ...
    function abs_series(R::($ctype), arr::Vector{T}, len::Int, prec::Int, var::VarName=:x; max_precision::Int=prec, cached::Bool=true) where {T}
    ...
    function +(a::($etype), b::($etype))
      ...
      z = parent(a)()
      z.prec = prec
      ccall(($(flint_fn * "_add_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int, Ref{($ctype)}), z, a, b, lenz, base_ring(a))
      return z
    end
    ...
    function -(a::($etype), b::($etype))
      ...
      z = parent(a)()
      z.prec = prec
      ccall(($(flint_fn * "_sub_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int, Ref{($ctype)}), z, a, b, lenz, base_ring(a))
      return z
    end
    ...
    *(x::($etype), y::($btype)) = y * x
    ...
    function shift_left(x::($etype), len::Int)
      ...
      ccall(($(flint_fn * "_shift_left"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int, Ref{($ctype)}), z, x, len, base_ring(x))
      ccall(($(flint_fn * "_set_trunc"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int, Ref{($ctype)}), z, z, zlen, base_ring(x))
      return z
    end
    ...
    function shift_right(x::($etype), len::Int)
      ...
      if len >= xlen
        ...
        z.prec = x.prec - len
        ccall(($(flint_fn * "_shift_right"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int, Ref{($ctype)}), z, x, len, base_ring(x))
      end
      return z
    end
    ...
    function truncate(x::($etype), prec::Int)
      ...
      z = parent(x)()
      z.prec = prec
      ccall(($(flint_fn * "_set_trunc"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int, Ref{($ctype)}), z, x, prec, base_ring(x))
      return z
    end
    ...
    function ==(x::($etype), y::($etype))
      ...
      n = min(n, prec)
      return Bool(ccall(($(flint_fn * "_equal_trunc"), libflint), Cint, (Ref{($etype)}, Ref{($etype)}, Int, Ref{($ctype)}), x, y, n, base_ring(x)))
    end
    ...
    function ==(x::($etype), y::($btype))
      ...
      elseif length(x) == 1
        z = base_ring(x)()
        ccall(($(flint_fn * "_get_coeff"), libflint), Nothing, (Ref{($btype)}, Ref{($etype)}, Int, Ref{($ctype)}), z, x, 0, base_ring(x))
        return z == y
      else
    ...
    function zero!(z::($etype))
      ccall(($(flint_fn * "_zero"), libflint), Nothing, (Ref{($etype)}, Ref{($ctype)}), z, base_ring(z))
    ...
    function addeq!(a::($etype), b::($etype))
      ...
      lenz = max(lena, lenb)
      a.prec = prec
      ccall(($(flint_fn * "_add_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int, Ref{($ctype)}), a, a, b, lenz, base_ring(a))
      return a
    end
    ...
    function add!(c::($etype), a::($etype), b::($etype))
      ...
      lenc = max(lena, lenb)
      c.prec = prec
      ccall(($(flint_fn * "_add_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int, Ref{($ctype)}), c, a, b, lenc, base_ring(a))
      return c
    end
