...
for (etype, rtype, mtype, brtype, flint_fn) in
    ((zzModRelPowerSeriesRingElem, zzModRelPowerSeriesRing, zzModRingElem, zzModRing, "nmod_poly"), (fpRelPowerSeriesRingElem, fpRelPowerSeriesRing, fpFieldElem, fpField, "nmod_poly"))
  @eval begin
    ...
    function normalise(a::($etype), len::Int)
      if len > 0
        c = ccall(($(flint_fn * "_get_coeff_ui"), libflint), UInt, (Ref{($etype)}, Int), a, len - 1)
      end
    ...
    function polcoeff(x::($etype), n::Int)
      ...
      z = ccall(($(flint_fn * "_get_coeff_ui"), libflint), UInt, (Ref{($etype)}, Int), x, n)
      return R(z)
    end
    ...
    function renormalize!(z::($etype))
      ...
      if i == zlen
        ...
        z.val = zval + i
        ccall(($(flint_fn * "_shift_right"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, z, i)
      end
      return nothing
    end
    ...
    function -(x::($etype))
      z = parent(x)()
      ccall(($(flint_fn * "_neg"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}), z, x)
      z.prec = x.prec
      z.val = x.val
      return z
    end
    ...
    function +(a::($etype), b::($etype))
      ...
      if a.val < b.val
        ...
        ccall(($(flint_fn * "_set_trunc"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, b, max(0, lenz - b.val + a.val))
        ccall(($(flint_fn * "_shift_left"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, z, b.val - a.val)
        ccall(($(flint_fn * "_add_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int), z, z, a, lenz)
      elseif b.val < a.val
        lenz = max(lena + a.val - b.val, lenb)
        ccall(($(flint_fn * "_set_trunc"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, a, max(0, lenz - a.val + b.val))
        ccall(($(flint_fn * "_shift_left"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, z, a.val - b.val)
        ccall(($(flint_fn * "_add_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int), z, z, b, lenz)
      else
        lenz = max(lena, lenb)
        ccall(($(flint_fn * "_add_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int), z, a, b, lenz)
      end
      z.prec = prec
      z.val = val
      renormalize!(z)
    ...
    function -(a::($etype), b::($etype))
      ...
      if a.val < b.val
        ...
        ccall(($(flint_fn * "_set_trunc"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, b, max(0, lenz - b.val + a.val))
        ccall(($(flint_fn * "_shift_left"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, z, b.val - a.val)
        ccall(($(flint_fn * "_neg"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}), z, z)
        ccall(($(flint_fn * "_add_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int), z, z, a, lenz)
      elseif b.val < a.val
        lenz = max(lena + a.val - b.val, lenb)
        ccall(($(flint_fn * "_set_trunc"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, a, max(0, lenz - a.val + b.val))
        ccall(($(flint_fn * "_shift_left"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, z, a.val - b.val)
        ccall(($(flint_fn * "_sub_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int), z, z, b, lenz)
      else
        lenz = max(lena, lenb)
        ccall(($(flint_fn * "_sub_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int), z, a, b, lenz)
      end
      z.prec = prec
      z.val = val
      renormalize!(z)
    ...
    function *(a::($etype), b::($etype))
      ...
      ccall(($(flint_fn * "_mullow"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int), z, a, b, lenz)
      renormalize!(z)
      return z
    end
    ...
    function *(x::($mtype), y::($etype))
      ...
      z.prec = y.prec
      z.val = y.val
      ccall(($(flint_fn * "_scalar_mul_nmod"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, UInt), z, y, data(x))
      renormalize!(z)
      return z
    end
    ...
    *(x::($etype), y::($mtype)) = y * x
    ...
    function *(x::ZZRingElem, y::($etype))
      ...
      ccall(($(flint_fn * "_scalar_mul_nmod"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, UInt), z, y, r)
      renormalize!(z)
      return z
    end
    ...
    function *(x::UInt, y::($etype))
      ...
      z.prec = y.prec
      z.val = y.val
      ccall(($(flint_fn * "_scalar_mul_nmod"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, UInt), z, y, mod(x, modulus(y)))
      renormalize!(z)
      return z
    end
    ...
    *(x::($etype), y::Integer) = y * x
    ...
    function shift_right(x::($etype), len::Int)
      ...
      if len >= xlen + xval
        ...
        ccall(($(flint_fn * "_shift_right"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, x, xlen - zlen)
        renormalize!(z)
      end
      return z
    end
    ...
    function truncate(x::($etype), prec::Int)
      ...
      if prec <= xval
        ...
        z.val = xval
        ccall(($(flint_fn * "_set_trunc"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, x, min(prec - xval, xlen))
      end
      return z
    end
    ...
    function ^(a::($etype), b::Int)
      ...
      elseif b == 0
        ...
        z.val = b * valuation(a)
        ccall(($(flint_fn * "_pow_trunc"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int, Int), z, a, b, z.prec - z.val)
      end
      renormalize!(z)
      return z
    end
    ...
    function ==(x::($etype), y::($etype))
      ...
      return Bool(ccall(($(flint_fn * "_equal_trunc"), libflint), Cint, (Ref{($etype)}, Ref{($etype)}, Int), x, y, xlen))
    end
    ...
    function isequal(x::($etype), y::($etype))
      ...
      return Bool(ccall(($(flint_fn * "_equal"), libflint), Cint, (Ref{($etype)}, Ref{($etype)}), x, y))
    end
    ...
    function ==(x::($etype), y::($mtype))
      ...
      elseif pol_length(x) == 1
        if x.val == 0
          z = ccall(($(flint_fn * "_get_coeff_ui"), libflint), UInt, (Ref{($etype)}, Int), x, 0)
          return data(y) == z
        else
          return false
        end
    ...
    function divexact(x::($etype), y::($etype); check::Bool=true)
      ...
      z.prec = prec + z.val
      if prec != 0
        ccall(($(flint_fn * "_div_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int), z, x, y, prec)
      end
      return z
    end
    ...
    function divexact(x::($etype), y::($mtype); check::Bool=true)
      ...
      z.val = x.val
      r = inv(y)
      ccall(($(flint_fn * "_scalar_mul_nmod"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, UInt), z, x, data(r))
      return z
    end
    ...
    function divexact(x::($etype), y::ZZRingElem; check::Bool=true)
      ...
      ccall(($(flint_fn * "_scalar_mul_nmod"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, UInt), z, x, data(rinv))
      return z
    end
    ...
    function inv(a::($etype))
      ...
      ainv.prec = a.prec
      ainv.val = 0
      ccall(($(flint_fn * "_inv_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), ainv, a, a.prec)
      return ainv
    end
    ...
    function Base.exp(a::($etype))
      ...
      ccall(($("_" * flint_fn * "_set_length"), libflint), Nothing, (Ref{($etype)}, Int), z, normalise(z, preca))
      return z
    end
    ...
    function zero!(x::($etype))
      ccall(($(flint_fn * "_zero"), libflint), Nothing, (Ref{($etype)},), x)
    ...
    function fit!(x::($etype), n::Int)
      ccall(($(flint_fn * "_fit_length"), libflint), Nothing, (Ref{($etype)}, Int), x, n)
      return nothing
    end

    function setcoeff!(z::($etype), n::Int, x::ZZRingElem)
      r = ccall((:fmpz_fdiv_ui, libflint), UInt, (Ref{ZZRingElem}, UInt), x, modulus(z))
      ccall(($(flint_fn * "_set_coeff_ui"), libflint), Nothing, (Ref{($etype)}, Int, UInt), z, n, r)
      return z
    end
    ...
    function setcoeff!(z::($etype), n::Int, x::($mtype))
      ccall(($(flint_fn * "_set_coeff_ui"), libflint), Nothing, (Ref{($etype)}, Int, UInt), z, n, data(x))
      return z
    end
    ...
    function addeq!(a::($etype), b::($etype))
      ...
      if a.val < b.val
        ...
        ccall(($(flint_fn * "_set_trunc"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, b, max(0, lenz - b.val + a.val))
        ccall(($(flint_fn * "_shift_left"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, z, b.val - a.val)
        ccall(($(flint_fn * "_add_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int), a, a, z, lenz)
      elseif b.val < a.val
        lenz = max(lena + a.val - b.val, lenb)
        ccall(($(flint_fn * "_truncate"), libflint), Nothing, (Ref{($etype)}, Int), a, max(0, lenz - a.val + b.val))
        ccall(($(flint_fn * "_shift_left"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), a, a, a.val - b.val)
        ccall(($(flint_fn * "_add_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int), a, a, b, lenz)
      else
        lenz = max(lena, lenb)
        ccall(($(flint_fn * "_add_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int), a, a, b, lenz)
      end
    ...
    function add!(c::($etype), a::($etype), b::($etype))
      ...
      if a.val < b.val
        ...
        ccall(($(flint_fn * "_set_trunc"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), c, b, max(0, lenc - b.val + a.val))
        ccall(($(flint_fn * "_shift_left"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), c, c, b.val - a.val)
        ccall(($(flint_fn * "_add_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int), c, c, a, lenc)
      elseif b.val < a.val
        lenc = max(lena + a.val - b.val, lenb)
        ccall(($(flint_fn * "_set_trunc"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), c, a, max(0, lenc - a.val + b.val))
        ccall(($(flint_fn * "_shift_left"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), c, c, a.val - b.val)
        ccall(($(flint_fn * "_add_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int), c, c, b, lenc)
      else
        lenc = max(lena, lenb)
        ccall(($(flint_fn * "_add_series"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Int), c, a, b, lenc)
      end
      c.prec = prec
      c.val = val
      renormalize!(c)
    ...
    function set_length!(a::($etype), n::Int)
      ccall(($("_" * flint_fn * "_set_length"), libflint), Nothing, (Ref{$(etype)}, Int), a, n::Int)
      return a
    end
...
function sqrt_classical(a::fpRelPowerSeriesRingElem; check::Bool=true)
  ...
  a = divexact(a, c)
  ccall((:nmod_poly_sqrt_series, libflint), Nothing, (Ref{fpRelPowerSeriesRingElem}, Ref{fpRelPowerSeriesRingElem}, Int), z, a, a.prec)
  if !isone(s)
    z *= s
  end
  return true, z
