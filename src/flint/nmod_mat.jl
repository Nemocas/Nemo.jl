...
@inline function getindex(a::zzModMatrix, i::Int, j::Int)
  ...
  u = ccall((:nmod_mat_get_entry, libflint), UInt, (Ref{zzModMatrix}, Int, Int), a, i - 1, j - 1)
...
function getindex_raw(a::T, i::Int, j::Int) where {T<:Zmodn_mat}
  return ccall((:nmod_mat_get_entry, libflint), UInt, (Ref{T}, Int, Int), a, i - 1, j - 1)
end
...
setindex!(a::T, u::Integer, i::Int, j::Int) where {T<:Zmodn_mat} = setindex!(a, ZZRingElem(u), i, j)
...
function setindex_raw!(a::T, u::UInt, i::Int, j::Int) where {T<:Zmodn_mat}
  ccall((:nmod_mat_set_entry, libflint), Nothing, (Ref{T}, Int, Int, UInt), a, i - 1, j - 1, u)
end

# as per setindex! but no reduction mod n and no bounds checking
function setindex_raw!(a::T, u::ZZRingElem, i::Int, j::Int) where {T<:Zmodn_mat}
  t = ZZRingElem()
  ccall((:fmpz_mod_ui, libflint), UInt, (Ref{ZZRingElem}, Ref{ZZRingElem}, UInt), t, u, a.n)
...
function setindex!(a::zzModMatrix, b::zzModMatrix, r::UnitRange{Int64}, c::UnitRange{Int64})
  ...
  A = view(a, r, c)
  ccall((:nmod_mat_set, libflint), Nothing, (Ref{zzModMatrix}, Ref{zzModMatrix}), A, b)
end
...
function deepcopy_internal(a::zzModMatrix, dict::IdDict)
  ...
  ccall((:nmod_mat_set, libflint), Nothing, (Ref{zzModMatrix}, Ref{zzModMatrix}), z, a)
  return z
end
...
==(a::T, b::T) where {T<:Zmodn_mat} = (a.base_ring == b.base_ring) && Bool(ccall((:nmod_mat_equal, libflint), Cint, (Ref{T}, Ref{T}), a, b))
...
function transpose(a::T) where {T<:Zmodn_mat}
  ...
  ccall((:nmod_mat_transpose, libflint), Nothing, (Ref{T}, Ref{T}), z, a)
  return z
end

function transpose!(a::T) where {T<:Zmodn_mat}
  !is_square(a) && error("Matrix must be a square matrix")
  ccall((:nmod_mat_transpose, libflint), Nothing, (Ref{T}, Ref{T}), a, a)
end
...
function swap_rows!(x::T, i::Int, j::Int) where {T<:Zmodn_mat}
  ccall((:nmod_mat_swap_rows, libflint), Nothing, (Ref{T}, Ptr{Nothing}, Int, Int), x, C_NULL, i - 1, j - 1)
  return x
end
...
function swap_cols!(x::T, i::Int, j::Int) where {T<:Zmodn_mat}
  ccall((:nmod_mat_swap_cols, libflint), Nothing, (Ref{T}, Ptr{Nothing}, Int, Int), x, C_NULL, i - 1, j - 1)
  return x
end
...
function reverse_rows!(x::T) where {T<:Zmodn_mat}
  ccall((:nmod_mat_invert_rows, libflint), Nothing, (Ref{T}, Ptr{Nothing}), x, C_NULL)
  return x
end
...
function reverse_cols!(x::T) where {T<:Zmodn_mat}
  ccall((:nmod_mat_invert_cols, libflint), Nothing, (Ref{T}, Ptr{Nothing}), x, C_NULL)
  return x
end
...
function -(x::T) where {T<:Zmodn_mat}
  z = similar(x)
  ccall((:nmod_mat_neg, libflint), Nothing, (Ref{T}, Ref{T}), z, x)
  return z
end
...
function +(x::T, y::T) where {T<:Zmodn_mat}
  check_parent(x, y)
  z = similar(x)
  ccall((:nmod_mat_add, libflint), Nothing, (Ref{T}, Ref{T}, Ref{T}), z, x, y)
  return z
end
...
function -(x::T, y::T) where {T<:Zmodn_mat}
  check_parent(x, y)
  z = similar(x)
  ccall((:nmod_mat_sub, libflint), Nothing, (Ref{T}, Ref{T}, Ref{T}), z, x, y)
  return z
end
...
function *(x::T, y::T) where {T<:Zmodn_mat}
  ...
  ccall((:nmod_mat_mul, libflint), Nothing, (Ref{T}, Ref{T}, Ref{T}), z, x, y)
  return z
end
...
function mul!(z::Vector{UInt}, a::T, b::Vector{UInt}) where {T<:Zmodn_mat}
  ccall((:nmod_mat_mul_nmod_vec, libflint), Nothing, (Ptr{UInt}, Ref{T}, Ptr{UInt}, Int), z, a, b, length(b))
  return z
end

# entries of a required to be in [0,n)
function mul!(z::Vector{UInt}, a::Vector{UInt}, b::T) where {T<:Zmodn_mat}
  ccall((:nmod_mat_nmod_vec_mul, libflint), Nothing, (Ptr{UInt}, Ptr{UInt}, Int, Ref{T}), z, a, length(a), b)
  return z
end

function mul!(a::zzModMatrix, b::zzModMatrix, c::zzModRingElem)
  ccall((:nmod_mat_scalar_mul, libflint), Nothing, (Ref{zzModMatrix}, Ref{zzModMatrix}, UInt), a, b, c.data)
  return a
end
...
function Generic.add_one!(a::zzModMatrix, i::Int, j::Int)
  @boundscheck Generic._checkbounds(a, i, j)
  x = ccall((:nmod_mat_get_entry, libflint), UInt, (Ref{zzModMatrix}, Int, Int), a, i - 1, j - 1)
  x += 1
  if x == base_ring(a).n
    x = UInt(0)
  end
...
function *(x::T, y::UInt) where {T<:Zmodn_mat}
  z = similar(x)
  ccall((:nmod_mat_scalar_mul, libflint), Nothing, (Ref{T}, Ref{T}, UInt), z, x, y)
  return z
end
...
function *(x::T, y::ZZRingElem) where {T<:Zmodn_mat}
  t = ZZRingElem()
  ccall((:fmpz_mod_ui, libflint), UInt, (Ref{ZZRingElem}, Ref{ZZRingElem}, UInt), t, y, x.n)
  tt = ccall((:fmpz_get_ui, libflint), UInt, (Ref{ZZRingElem},), t)
  return x * tt
end
...
function ^(x::T, y::UInt) where {T<:Zmodn_mat}
  z = similar(x)
  ccall((:nmod_mat_pow, libflint), Nothing, (Ref{T}, Ref{T}, UInt), z, x, y)
  return z
end
...
function ^(x::T, y::ZZRingElem) where {T<:Zmodn_mat}
  (y < 0) && error("Exponent must be positive")
  (y > ZZRingElem(typemax(UInt))) && error("Exponent must be smaller than ", ZZRingElem(typemax(UInt)))
  return x^(UInt(y))
end
...
function inv(a::zzModMatrix)
  ...
  if is_prime(a.n)
    z = similar(a)
    r = ccall((:nmod_mat_inv, libflint), Int, (Ref{zzModMatrix}, Ref{zzModMatrix}), z, a)
...
function Solve._can_solve_internal_no_check(A::zzModMatrix, b::zzModMatrix, task::Symbol; side::Symbol=:left)
  ...
  fl = ccall((:nmod_mat_can_solve, libflint), Cint, (Ref{zzModMatrix}, Ref{zzModMatrix}, Ref{zzModMatrix}), x, A, b)
...
function lu!(P::Generic.Perm, x::T) where {T<:Zmodn_mat}
  ...
  rank = Int(ccall((:nmod_mat_lu, libflint), Cint, (Ptr{Int}, Ref{T}, Cint), P.d, x, 0))
...
function Base.view(x::zzModMatrix, r1::Int, c1::Int, r2::Int, c2::Int)
  ...
  z.view_parent = x
  ccall((:nmod_mat_window_init, libflint), Nothing, (Ref{zzModMatrix}, Ref{zzModMatrix}, Int, Int, Int, Int), z, x, r1 - 1, c1 - 1, r2, c2)
...
function hcat(x::T, y::T) where {T<:Zmodn_mat}
  ...
  ccall((:nmod_mat_concat_horizontal, libflint), Nothing, (Ref{T}, Ref{T}, Ref{T}), z, x, y)
  return z
end
...
function vcat(x::T, y::T) where {T<:Zmodn_mat}
  ...
  ccall((:nmod_mat_concat_vertical, libflint), Nothing, (Ref{T}, Ref{T}, Ref{T}), z, x, y)
  return z
end
...
function lift(a::T) where {T<:Zmodn_mat}
  ...
  ccall((:fmpz_mat_set_nmod_mat, libflint), Nothing, (Ref{ZZMatrix}, Ref{T}), z, a)
  return z
end
...
function charpoly(R::zzModPolyRing, a::zzModMatrix)
  m = deepcopy(a)
  p = R()
  ccall((:nmod_mat_charpoly, libflint), Nothing, (Ref{zzModPolyRingElem}, Ref{zzModMatrix}), p, m)
  return p
end
...
function minpoly(R::zzModPolyRing, a::zzModMatrix)
  p = R()
  ccall((:nmod_mat_minpoly, libflint), Nothing, (Ref{zzModPolyRingElem}, Ref{zzModMatrix}), p, a)
  return p
end
...
function nullspace(M::zzModMatrix)
  ...
  nullity = ccall((:nmod_mat_nullspace, libflint), Int, (Ref{zzModMatrix}, Ref{zzModMatrix}), N, M)
