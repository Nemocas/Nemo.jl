...
function check_parent(a::FlintPuiseuxSeriesElem, b::FlintPuiseuxSeriesElem)
  parent(a) != parent(b) && error("Incompatible Puiseux series rings in Puiseux series operation")
end
...
function expressify(a::FlintPuiseuxSeriesElem, x=var(parent(a.data)); context=nothing)
  sum = Expr(:call, :+)
  ...
  push!(sum.args, Expr(:call, :O, Expr(:call, :^, x, expressify(q; context=context))))
  return sum
end
...
RandomExtensions.make(S::FlintPuiseuxSeriesRingOrField, val_range::AbstractUnitRange{Int}, scale_range::AbstractUnitRange{Int}, vs...) = make(S, scale_range, make(laurent_ring(S), val_range, vs...))

function rand(rng::AbstractRNG, sp::SamplerTrivial{<:Make3{<:RingElement,<:FlintPuiseuxSeriesRingOrField,<:AbstractUnitRange{Int}}})
...
function rand(rng::AbstractRNG, S::FlintPuiseuxSeriesRingOrField, val_range::AbstractUnitRange{Int}, scale_range::AbstractUnitRange{Int}, v...)
  rand(rng, make(S, val_range, scale_range, v...))
end

rand(S::FlintPuiseuxSeriesRingOrField, val_range, scale_range, v...) = rand(Random.GLOBAL_RNG, S, val_range, scale_range, v...)
