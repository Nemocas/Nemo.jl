...
length(x::T) where {T<:Zmodn_poly} = ccall((:nmod_poly_length, libflint), Int, (Ref{T},), x)

degree(x::T) where {T<:Zmodn_poly} = ccall((:nmod_poly_degree, libflint), Int, (Ref{T},), x)

function coeff(x::T, n::Int) where {T<:Zmodn_poly}
  n < 0 && throw(DomainError(n, "Index must be non-negative"))
  return base_ring(x)(ccall((:nmod_poly_get_coeff_ui, libflint), UInt, (Ref{T}, Int), x, n))
end

function coeff_raw(x::T, n::Int) where {T<:Zmodn_poly}
  return ccall((:nmod_poly_get_coeff_ui, libflint), UInt, (Ref{T}, Int), x, n)
end
...
is_gen(a::T) where {T<:Zmodn_poly} = (degree(a) == 1 && iszero(coeff(a, 0)) && isone(coeff(a, 1)))

iszero(a::T) where {T<:Zmodn_poly} = Bool(ccall((:nmod_poly_is_zero, libflint), Int32, (Ref{T},), a))
...
function -(x::T) where {T<:Zmodn_poly}
  z = parent(x)()
  ccall((:nmod_poly_neg, libflint), Nothing, (Ref{T}, Ref{T}), z, x)
  return z
end
...
function +(x::T, y::T) where {T<:Zmodn_poly}
  check_parent(x, y)
  z = parent(x)()
  ccall((:nmod_poly_add, libflint), Nothing, (Ref{T}, Ref{T}, Ref{T}), z, x, y)
  return z
end
...
function -(x::T, y::T) where {T<:Zmodn_poly}
  check_parent(x, y)
  z = parent(x)()
  ccall((:nmod_poly_sub, libflint), Nothing, (Ref{T}, Ref{T}, Ref{T}), z, x, y)
  return z
end
...
function *(x::T, y::T) where {T<:Zmodn_poly}
  check_parent(x, y)
  z = parent(x)()
  ccall((:nmod_poly_mul, libflint), Nothing, (Ref{T}, Ref{T}, Ref{T}), z, x, y)
  return z
end
...
function *(x::T, y::UInt) where {T<:Zmodn_poly}
  z = parent(x)()
  ccall((:nmod_poly_scalar_mul_nmod, libflint), Nothing, (Ref{T}, Ref{T}, UInt), z, x, y)
  return z
end
...
function *(x::T, y::ZZRingElem) where {T<:Zmodn_poly}
  ...
  t = ZZRingElem()
  tt = UInt(0)
  ccall((:fmpz_mod_ui, libflint), UInt, (Ref{ZZRingElem}, Ref{ZZRingElem}, UInt), t, y, parent(x).n)
...
function +(x::T, y::UInt) where {T<:Zmodn_poly}
  z = parent(x)()
  ccall((:nmod_poly_add_ui, libflint), Nothing, (Ref{T}, Ref{T}, UInt), z, x, y)
  return z
end
...
function -(x::T, y::UInt) where {T<:Zmodn_poly}
  z = parent(x)()
  ccall((:nmod_poly_sub_ui, libflint), Nothing, (Ref{T}, Ref{T}, UInt), z, x, y)
  return z
end
...
function ^(x::T, y::Int) where {T<:Zmodn_poly}
  ...
  z = parent(x)()
  ccall((:nmod_poly_pow, libflint), Nothing, (Ref{T}, Ref{T}, Int), z, x, y)
  return z
end
...
function ==(x::T, y::T) where {T<:Zmodn_poly}
  check_parent(x, y)
  return Bool(ccall((:nmod_poly_equal, libflint), Int32, (Ref{T}, Ref{T}), x, y))
end
...
function ==(x::zzModPolyRingElem, y::zzModRingElem)
  ...
  if length(x) > 1
    return false
  elseif length(x) == 1
    u = ccall((:nmod_poly_get_coeff_ui, libflint), UInt, (Ref{zzModPolyRingElem}, Int), x, 0)
    return u == y
  else
    return iszero(y)
  end
...
function truncate(a::T, n::Int) where {T<:Zmodn_poly}
  ...
  ccall((:nmod_poly_truncate, libflint), Nothing, (Ref{T}, Int), z, n)
  return z
end
...
function mullow(x::T, y::T, n::Int) where {T<:Zmodn_poly}
  ...
  n < 0 && throw(DomainError(n, "Index must be non-negative"))
  z = parent(x)()
  ccall((:nmod_poly_mullow, libflint), Nothing, (Ref{T}, Ref{T}, Ref{T}, Int), z, x, y, n)
  return z
end
...
function reverse(x::T, len::Int) where {T<:Zmodn_poly}
  ...
  z = parent(x)()
  ccall((:nmod_poly_reverse, libflint), Nothing, (Ref{T}, Ref{T}, Int), z, x, len)
  return z
end
...
function shift_left(x::T, len::Int) where {T<:Zmodn_poly}
  ...
  z = parent(x)()
  ccall((:nmod_poly_shift_left, libflint), Nothing, (Ref{T}, Ref{T}, Int), z, x, len)
  return z
end
...
function shift_right(x::T, len::Int) where {T<:Zmodn_poly}
  len < 0 && throw(DomainError(len, "Shift must be non-negative."))
  z = parent(x)()
  ccall((:nmod_poly_shift_right, libflint), Nothing, (Ref{T}, Ref{T}, Int), z, x, len)
  return z
end
...
function divexact(x::zzModPolyRingElem, y::zzModPolyRingElem; check::Bool=true)
  ...
  z = parent(x)()
  ccall((:nmod_poly_div, libflint), Nothing, (Ref{zzModPolyRingElem}, Ref{zzModPolyRingElem}, Ref{zzModPolyRingElem}), z, x, y)
  return z
end
...
function Base.divrem(x::zzModPolyRingElem, y::zzModPolyRingElem)
  ...
  q = parent(x)()
  r = parent(x)()
  ccall((:nmod_poly_divrem, libflint), Nothing, (Ref{zzModPolyRingElem}, Ref{zzModPolyRingElem}, Ref{zzModPolyRingElem}, Ref{zzModPolyRingElem}), q, r, x, y)
  return q, r
end
...
function Base.div(x::zzModPolyRingElem, y::zzModPolyRingElem)
  ...
  lead_is_unit_or_throw(y)
  q = parent(x)()
  ccall((:nmod_poly_div, libflint), Nothing, (Ref{zzModPolyRingElem}, Ref{zzModPolyRingElem}, Ref{zzModPolyRingElem}), q, x, y)
  return q
end
...
function rem(x::zzModPolyRingElem, y::zzModPolyRingElem)
  ...
  lead_is_unit_or_throw(y)
  z = parent(x)()
  ccall((:nmod_poly_rem, libflint), Nothing, (Ref{zzModPolyRingElem}, Ref{zzModPolyRingElem}, Ref{zzModPolyRingElem}), z, x, y)
  return z
end
...
function invmod(x::T, y::T) where {T<:Zmodn_poly}
  ...
  z = parent(x)()
  r = ccall((:nmod_poly_invmod, libflint), Int32, (Ref{T}, Ref{T}, Ref{T}), z, x, y)
...
function mulmod(x::T, y::T, z::T) where {T<:Zmodn_poly}
  ...
  check_parent(y, z)
  w = parent(x)()
  ccall((:nmod_poly_mulmod, libflint), Nothing, (Ref{T}, Ref{T}, Ref{T}, Ref{T}), w, x, y, z)
  return w
end
...
function powermod(x::T, e::Int, y::T) where {T<:Zmodn_poly}
  ...
  ccall((:nmod_poly_powmod_ui_binexp, libflint), Nothing, (Ref{T}, Ref{T}, Int, Ref{T}), z, x, e, y)
...
function resultant(x::zzModPolyRingElem, y::zzModPolyRingElem, check::Bool=true)
  ...
  r = ccall((:nmod_poly_resultant, libflint), UInt, (Ref{zzModPolyRingElem}, Ref{zzModPolyRingElem}), x, y)
...
function evaluate(x::zzModPolyRingElem, y::zzModRingElem)
  base_ring(x) != parent(y) && error("Elements must have same parent")
  z = ccall((:nmod_poly_evaluate_nmod, libflint), UInt, (Ref{zzModPolyRingElem}, UInt), x, y.data)
  return parent(y)(z)
end
...
function derivative(x::T) where {T<:Zmodn_poly}
  z = parent(x)()
  ccall((:nmod_poly_derivative, libflint), Nothing, (Ref{T}, Ref{T}), z, x)
  return z
end
...
function integral(x::T) where {T<:Zmodn_poly}
  z = parent(x)()
  ccall((:nmod_poly_integral, libflint), Nothing, (Ref{T}, Ref{T}), z, x)
  return z
end
...
function AbstractAlgebra._compose_right(x::T, y::T) where {T<:Zmodn_poly}
  check_parent(x, y)
  z = parent(x)()
  ccall((:nmod_poly_compose, libflint), Nothing, (Ref{T}, Ref{T}, Ref{T}), z, x, y)
  return z
end
...
function interpolate(R::zzModPolyRing, x::Vector{zzModRingElem}, y::Vector{zzModRingElem})
  z = R()
  ...
  ccall((:nmod_poly_interpolate_nmod_vec, libflint), Nothing, (Ref{zzModPolyRingElem}, Ptr{UInt}, Ptr{UInt}, Int), z, ax, ay, length(x))
  return z
end
...
function inflate(x::T, n::Int) where {T<:Zmodn_poly}
  ...
  z = parent(x)()
  ccall((:nmod_poly_inflate, libflint), Nothing, (Ref{T}, Ref{T}, UInt), z, x, UInt(n))
  return z
end
...
function deflate(x::T, n::Int) where {T<:Zmodn_poly}
  ...
  z = parent(x)()
  ccall((:nmod_poly_deflate, libflint), Nothing, (Ref{T}, Ref{T}, UInt), z, x, UInt(n))
  return z
end
...
function lift(R::ZZPolyRing, y::zzModPolyRingElem)
  z = ZZPolyRingElem()
  ccall((:fmpz_poly_set_nmod_poly_unsigned, libflint), Nothing, (Ref{ZZPolyRingElem}, Ref{zzModPolyRingElem}), z, y)
  z.parent = R
  return z
end
...
function is_irreducible(x::zzModPolyRingElem)
  ...
  return Bool(ccall((:nmod_poly_is_irreducible, libflint), Int32, (Ref{zzModPolyRingElem},), x))
end
...
function is_squarefree(x::zzModPolyRingElem)
  ...
  return Bool(ccall((:nmod_poly_is_squarefree, libflint), Int32, (Ref{zzModPolyRingElem},), x))
end
...
function _factor(x::zzModPolyRingElem)
  ...
  z = ccall((:nmod_poly_factor, libflint), UInt, (Ref{nmod_poly_factor}, Ref{zzModPolyRingElem}), fac, x)
  res = Dict{zzModPolyRingElem,Int}()
  for i in 1:(fac.num)
    f = parent(x)()
    ccall((:nmod_poly_factor_get_poly, libflint), Nothing, (Ref{zzModPolyRingElem}, Ref{nmod_poly_factor}, Int), f, fac, i - 1)
...
function _factor_squarefree(x::zzModPolyRingElem)
  fac = nmod_poly_factor(x.mod_n)
  ccall((:nmod_poly_factor_squarefree, libflint), UInt, (Ref{nmod_poly_factor}, Ref{zzModPolyRingElem}), fac, x)
  res = Dict{zzModPolyRingElem,Int}()
  for i in 1:(fac.num)
    f = parent(x)()
    ccall((:nmod_poly_factor_get_poly, libflint), Nothing, (Ref{zzModPolyRingElem}, Ref{nmod_poly_factor}, Int), f, fac, i - 1)
...
function factor_distinct_deg(x::zzModPolyRingElem)
  ...
  degss = [pointer(degs)]
  fac = nmod_poly_factor(x.mod_n)
  ccall((:nmod_poly_factor_distinct_deg, libflint), UInt, (Ref{nmod_poly_factor}, Ref{zzModPolyRingElem}, Ptr{Ptr{Int}}), fac, x, degss)
...
function roots(a::zzModPolyRingElem)
  ...
  if is_prime(n)
    ccall((:nmod_poly_roots, libflint), UInt, (Ref{nmod_poly_factor}, Ref{zzModPolyRingElem}, Cint), fac, a, 0)
  else
    nfac = n_factor()
    ccall((:n_factor, libflint), Nothing, (Ref{n_factor}, UInt), nfac, n)
    ccall((:nmod_poly_roots_factored, libflint), UInt, (Ref{nmod_poly_factor}, Ref{zzModPolyRingElem}, Cint, Ref{n_factor}), fac, a, 0, nfac)
  end
  f = R()
  res = zzModRingElem[]
  for i in 1:(fac.num)
...
function remove(z::zzModPolyRingElem, p::zzModPolyRingElem)
  ...
  z = deepcopy(z)
  v = ccall((:nmod_poly_remove, libflint), Int, (Ref{zzModPolyRingElem}, Ref{zzModPolyRingElem}), z, p)
  return v, z
end
...
function zero!(x::T) where {T<:Zmodn_poly}
  ccall((:nmod_poly_zero, libflint), Nothing, (Ref{T},), x)
  return x
end
...
function fit!(x::T, n::Int) where {T<:Zmodn_poly}
  ccall((:nmod_poly_fit_length, libflint), Nothing, (Ref{T}, Int), x, n)
  return nothing
end

function setcoeff!(x::T, n::Int, y::UInt) where {T<:Zmodn_poly}
  ccall((:nmod_poly_set_coeff_ui, libflint), Nothing, (Ref{T}, Int, UInt), x, n, y)
  return x
end

function setcoeff!(x::T, n::Int, y::Int) where {T<:Zmodn_poly}
  ccall((:nmod_poly_set_coeff_ui, libflint), Nothing, (Ref{T}, Int, UInt), x, n, mod(y, x.mod_n))
  return x
end
...
function setcoeff!(x::T, n::Int, y::ZZRingElem) where {T<:Zmodn_poly}
  ...
  ccall((:nmod_poly_set_coeff_ui, libflint), Nothing, (Ref{T}, Int, UInt), x, n, r)
  return x
end
...
function addeq!(z::T, y::T) where {T<:Zmodn_poly}
  ccall((:nmod_poly_add, libflint), Nothing, (Ref{T}, Ref{T}, Ref{T}), z, z, y)
  return z
end
