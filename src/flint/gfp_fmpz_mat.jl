...
function getindex!(v::FpFieldElem, a::FpMatrix, i::Int, j::Int)
  ...
  GC.@preserve a begin
    z = ccall((:fmpz_mod_mat_entry, libflint), Ptr{ZZRingElem}, (Ref{FpMatrix}, Int, Int), a, i - 1, j - 1)
...
@inline function getindex_raw(a::FpMatrix, i::Int, j::Int)
  u = ZZRingElem()
  ccall((:fmpz_mod_mat_get_entry, libflint), Nothing, (Ref{ZZRingElem}, Ref{FpMatrix}, Int, Int, Ref{fmpz_mod_ctx_struct}), u, a, i - 1, j - 1, base_ring(a).ninv)
  return u
end
...
@inline function setindex_raw!(a::FpMatrix, u::ZZRingElem, i::Int, j::Int)
  ccall((:fmpz_mod_mat_set_entry, libflint), Nothing, (Ref{FpMatrix}, Int, Int, Ref{ZZRingElem}, Ref{Nothing}), a, i - 1, j - 1, u, C_NULL) # ctx is not needed here
end
...
function setindex!(a::FpMatrix, b::FpMatrix, r::UnitRange{Int64}, c::UnitRange{Int64})
  ...
  A = view(a, r, c)
  ccall((:fmpz_mod_mat_set, libflint), Nothing, (Ref{FpMatrix}, Ref{FpMatrix}, Ref{Nothing}), A, b, C_NULL)
end
...
function deepcopy_internal(a::FpMatrix, dict::IdDict)
  ...
  ccall((:fmpz_mod_mat_set, libflint), Nothing, (Ref{FpMatrix}, Ref{FpMatrix}, Ref{fmpz_mod_ctx_struct}), z, a, base_ring(a).ninv)
  return z
end
...
function one(a::FpMatrixSpace)
  ...
  z = a()
  ccall((:fmpz_mod_mat_one, libflint), Nothing, (Ref{FpMatrix}, Ref{fmpz_mod_ctx_struct}), z, base_ring(a).ninv)
  return z
end

function iszero(a::FpMatrix)
  r = ccall((:fmpz_mod_mat_is_zero, libflint), Cint, (Ref{FpMatrix}, Ref{fmpz_mod_ctx_struct}), a, base_ring(a).ninv)
  return Bool(r)
end
...
function tr(a::FpMatrix)
  ...
  R = base_ring(a)
  r = ZZRingElem()
  ccall((:fmpz_mod_mat_trace, libflint), Nothing, (Ref{ZZRingElem}, Ref{FpMatrix}, Ref{fmpz_mod_ctx_struct}), r, a, base_ring(a).ninv)
...
function Base.view(x::FpMatrix, r1::Int, c1::Int, r2::Int, c2::Int)
  ...
  z.view_parent = x
  ccall((:fmpz_mod_mat_window_init, libflint), Nothing, (Ref{FpMatrix}, Ref{FpMatrix}, Int, Int, Int, Int, Ref{fmpz_mod_ctx_struct}), z, x, r1 - 1, c1 - 1, r2, c2, base_ring(x).ninv)
  finalizer(_gfp_fmpz_mat_window_clear_fn, z)
  return z
end

function _gfp_fmpz_mat_window_clear_fn(a::FpMatrix)
  ccall((:fmpz_mod_mat_window_clear, libflint), Nothing, (Ref{FpMatrix}, Ref{fmpz_mod_ctx_struct}), a, base_ring(a).ninv)
end
...
function inv(a::FpMatrix)
  !is_square(a) && error("Matrix must be a square matrix")
  z = similar(a)
  r = ccall((:fmpz_mod_mat_inv, libflint), Int, (Ref{FpMatrix}, Ref{FpMatrix}, Ref{fmpz_mod_ctx_struct}), z, a, base_ring(a).ninv)
...
function nullspace(M::FpMatrix)
  ...
  nullity = ccall((:fmpz_mod_mat_nullspace, libflint), Int, (Ref{FpMatrix}, Ref{FpMatrix}, Ref{fmpz_mod_ctx_struct}), N, M, base_ring(M).ninv)
...
function Solve._can_solve_internal_no_check(A::FpMatrix, b::FpMatrix, task::Symbol; side::Symbol=:left)
  ...
  fl = ccall((:fmpz_mod_mat_can_solve, libflint), Cint, (Ref{FpMatrix}, Ref{FpMatrix}, Ref{FpMatrix}, Ref{fmpz_mod_ctx_struct}), x, A, b, base_ring(x).ninv)
...
function _solve_tril_right_flint!(x::FpMatrix, L::FpMatrix, B::FpMatrix, unit::Bool)
  ccall((:fmpz_mod_mat_solve_tril, libflint), Nothing, (Ref{FpMatrix}, Ref{FpMatrix}, Ref{FpMatrix}, Cint, Ref{fmpz_mod_ctx_struct}), x, L, B, Cint(unit), base_ring(x).ninv)
  return nothing
end

function _solve_triu_right_flint!(x::FpMatrix, U::FpMatrix, B::FpMatrix, unit::Bool)
  ccall((:fmpz_mod_mat_solve_triu, libflint), Nothing, (Ref{FpMatrix}, Ref{FpMatrix}, Ref{FpMatrix}, Cint, Ref{fmpz_mod_ctx_struct}), x, U, B, Cint(unit), base_ring(x).ninv)
  return nothing
end
...
function lu!(P::Generic.Perm, x::FpMatrix)
  ...
  rank = ccall((:fmpz_mod_mat_lu, libflint), Int, (Ptr{Int}, Ref{FpMatrix}, Cint, Ref{fmpz_mod_ctx_struct}), P.d, x, Cint(false), base_ring(x).ninv)
...
@inline mat_entry_ptr(A::FpMatrix, i::Int, j::Int) = ccall((:fmpz_mod_mat_entry, libflint), Ptr{ZZRingElem}, (Ref{FpMatrix}, Int, Int), A, i - 1, j - 1)
