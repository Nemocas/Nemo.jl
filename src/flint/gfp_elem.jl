...
function *(x::fpFieldElem, y::fpFieldElem)
  check_parent(x, y)
  R = parent(x)
  d = ccall((:n_mulmod2_preinv, libflint), UInt, (UInt, UInt, UInt, UInt), x.data, y.data, R.n, R.ninv)
  return fpFieldElem(d, R)
end
...
function *(x::Int, y::fpFieldElem)
  R = parent(y)
  if x < 0
    d = ccall((:n_mulmod2_preinv, libflint), UInt, (UInt, UInt, UInt, UInt), UInt(-x), y.data, R.n, R.ninv)
    return -fpFieldElem(d, R)
  else
    d = ccall((:n_mulmod2_preinv, libflint), UInt, (UInt, UInt, UInt, UInt), UInt(x), y.data, R.n, R.ninv)
    return fpFieldElem(d, R)
...
function ^(x::fpFieldElem, y::Int)
  ...
  d = ccall((:n_powmod2_preinv, libflint), UInt, (UInt, Int, UInt, UInt), UInt(x.data), y, R.n, R.ninv)
  return fpFieldElem(d, R)
end
...
function inv(x::fpFieldElem)
  ...
  xinv = ccall((:n_invmod, libflint), UInt, (UInt, UInt), x.data, R.n)
...
function divexact(x::fpFieldElem, y::fpFieldElem; check::Bool=true)
  ...
  R = parent(x)
  yinv = ccall((:n_invmod, libflint), UInt, (UInt, UInt), y.data, R.n)
  d = ccall((:n_mulmod2_preinv, libflint), UInt, (UInt, UInt, UInt, UInt), x.data, yinv, R.n, R.ninv)
  return fpFieldElem(d, R)
end
...
Random.Sampler(::Type{RNG}, R::fpField, n::Random.Repetition) where {RNG<:AbstractRNG} = Random.SamplerSimple(R, Random.Sampler(RNG, UInt(0):(R.n - 1), n))

rand(rng::AbstractRNG, R::Random.SamplerSimple{fpField}) = fpFieldElem(rand(rng, R.data), R[])
...
function (R::fpField)(a::Int)
  ...
  if d >= n
    d = ccall((:n_mod2_preinv, libflint), UInt, (UInt, UInt, UInt), d, n, ninv)
  end
  return fpFieldElem(d, R)
end
...
function (R::fpField)(a::UInt)
  n = R.n
  ninv = R.ninv
  a = ccall((:n_mod2_preinv, libflint), UInt, (UInt, UInt, UInt), a, n, ninv)
  return fpFieldElem(a, R)
end

function (R::fpField)(a::ZZRingElem)
  d = ccall((:fmpz_fdiv_ui, libflint), UInt, (Ref{ZZRingElem}, UInt), a, R.n)
  return fpFieldElem(d, R)
end
...
function (R::fpField)(a::QQFieldElem)
  ...
  n = ccall((:fmpz_fdiv_ui, libflint), UInt, (Ref{ZZRingElem}, UInt), num, R.n)
  d = ccall((:fmpz_fdiv_ui, libflint), UInt, (Ref{ZZRingElem}, UInt), den, R.n)
  V = [UInt(0)]
