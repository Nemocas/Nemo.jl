...
for (etype, rtype, mtype, brtype, flint_fn) in
    ((zzModAbsPowerSeriesRingElem, zzModAbsPowerSeriesRing, zzModRingElem, zzModRing, "nmod_poly"), (fpAbsPowerSeriesRingElem, fpAbsPowerSeriesRing, fpFieldElem, fpField, "nmod_poly"))
  @eval begin
    ...
    function coeff_raw(x::($etype), n::Int)
      ...
      return ccall(($(flint_fn * "_get_coeff_ui"), libflint), UInt, (Ref{($etype)}, Int), x, n)
    end
    ...
    function is_gen(a::($etype))
      return precision(a) == 0 || (length(a) == 2 && isone(coeff(a, 1)) && iszero(coeff(a, 0)))
    end
    ...
    function isone(a::($etype))
      return precision(a) == 0 || Bool(ccall(($(flint_fn * "_is_one"), libflint), Cint, (Ref{($etype)},), a))
    end
    ...
    function *(x::$(mtype), y::($etype))
      z = parent(y)()
      z.prec = y.prec
      ccall(($(flint_fn * "_scalar_mul_nmod"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, UInt), z, y, x.data)
      return z
    end
    ...
    *(x::($etype), y::$(mtype)) = y * x
    ...
    function *(x::ZZRingElem, y::($etype))
      R = base_ring(y)
      xmod = ccall((:fmpz_fdiv_ui, libflint), UInt, (Ref{ZZRingElem}, UInt), x, R.n)
      return R(xmod) * y
    end
    ...
    *(x::($etype), y::Integer) = y * x
    ...
    function shift_left(x::($etype), len::Int)
      ...
      ccall(($(flint_fn * "_shift_left"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, x, len)
      ccall(($(flint_fn * "_set_trunc"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, z, zlen)
      return z
    end
    ...
    function shift_right(x::($etype), len::Int)
      ...
      if len >= xlen
        ...
        z.prec = x.prec - len
        ccall(($(flint_fn * "_shift_right"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, x, len)
      end
      return z
    end
    ...
    function truncate(x::($etype), prec::Int)
      ...
      z = parent(x)()
      z.prec = prec
      ccall(($(flint_fn * "_set_trunc"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int), z, x, prec)
      return z
    end
    ...
    function ^(a::($etype), b::Int)
      ...
      elseif b == 0
        ...
        ccall(($(flint_fn * "_pow_trunc"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, UInt, Int), z, a, b, z.prec)
      end
      return z
    end
    ...
    function ==(x::($etype), y::($etype))
      ...
      return Bool(ccall(($(flint_fn * "_equal_trunc"), libflint), Cint, (Ref{($etype)}, Ref{($etype)}, Int), x, y, n))
    end
    ...
    function isequal(x::($etype), y::($etype))
      ...
      return Bool(ccall(($(flint_fn * "_equal"), libflint), Cint, (Ref{($etype)}, Ref{($etype)}, Int), x, y, length(x)))
    end
    ...
    function ==(x::($etype), y::ZZRingElem)
      R = base_ring(x)
      ymod = ccall((:fmpz_fdiv_ui, libflint), UInt, (Ref{ZZRingElem}, UInt), y, modulus(x))
      return x == R(ymod)
    end
    ...
    function divexact(x::($etype), y::$(mtype); check::Bool=true)
      ...
      z.prec = x.prec
      yinv = inv(y)
      ccall(($(flint_fn * "_scalar_mul_nmod"), libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, UInt), z, x, yinv.data)
      return z
    end
    ...
    function zero!(z::($etype))
      ccall(($(flint_fn * "_zero"), libflint), Nothing, (Ref{($etype)},), z)
      z.prec = parent(z).prec_max
      return z
    end

    function fit!(z::($etype), n::Int)
      ccall(($(flint_fn * "_fit_length"), libflint), Nothing, (Ref{($etype)}, Int), z, n)
      return nothing
    end

    function setcoeff!(z::($etype), n::Int, x::($mtype))
      ccall(($(flint_fn * "_set_coeff_ui"), libflint), Nothing, (Ref{($etype)}, Int, UInt), z, n, x.data)
      return z
    end
...
function sqrt_classical(a::fpAbsPowerSeriesRingElem; check::Bool=true)
  ...
  a = divexact(a, c)
  z.prec = a.prec - div(v, 2)
  ccall((:nmod_poly_sqrt_series, libflint), Nothing, (Ref{fpAbsPowerSeriesRingElem}, Ref{fpAbsPowerSeriesRingElem}, Int), z, a, a.prec)
  if !isone(s)
    z *= s
  end
  if !iszero(v)
