...
@inline function getindex(a::fpMatrix, i::Int, j::Int)
  ...
  u = ccall((:nmod_mat_get_entry, libflint), UInt, (Ref{fpMatrix}, Int, Int), a, i - 1, j - 1)
...
function setindex!(a::fpMatrix, b::fpMatrix, r::UnitRange{Int64}, c::UnitRange{Int64})
  ...
  A = view(a, r, c)
  ccall((:nmod_mat_set, libflint), Nothing, (Ref{fpMatrix}, Ref{fpMatrix}), A, b)
end
...
function deepcopy_internal(a::fpMatrix, dict::IdDict)
  ...
  ccall((:nmod_mat_set, libflint), Nothing, (Ref{fpMatrix}, Ref{fpMatrix}), z, a)
  return z
end
...
@inline function is_zero_entry(A::fpMatrix, i::Int, j::Int)
  @boundscheck Generic._checkbounds(A, i, j)
  x = ccall((:nmod_mat_get_entry, libflint), UInt, (Ref{fpMatrix}, Int, Int), A, i - 1, j - 1)
  return x == 0
end
...
function Generic.add_one!(a::fpMatrix, i::Int, j::Int)
  ...
  ccall((:nmod_mat_set_entry, libflint), Nothing, (Ref{fpMatrix}, Int, Int, UInt), a, i - 1, j - 1, x)
  return a
end
...
function Base.view(x::fpMatrix, r1::Int, c1::Int, r2::Int, c2::Int)
  ...
  z.view_parent = x
  ccall((:nmod_mat_window_init, libflint), Nothing, (Ref{fpMatrix}, Ref{fpMatrix}, Int, Int, Int, Int), z, x, r1 - 1, c1 - 1, r2, c2)
...
function lift(a::fpMatrix)
  ...
  ccall((:fmpz_mat_set_nmod_mat, libflint), Nothing, (Ref{ZZMatrix}, Ref{fpMatrix}), z, a)
  return z
end
...
function charpoly(R::fpPolyRing, a::fpMatrix)
  m = deepcopy(a)
  p = R()
  ccall((:nmod_mat_charpoly, libflint), Nothing, (Ref{fpPolyRingElem}, Ref{fpMatrix}), p, m)
  return p
end
...
function minpoly(R::fpPolyRing, a::fpMatrix)
  p = R()
  ccall((:nmod_mat_minpoly, libflint), Nothing, (Ref{fpPolyRingElem}, Ref{fpMatrix}), p, a)
  return p
end
...
function inv(a::fpMatrix)
  !is_square(a) && error("Matrix must be a square matrix")
  z = similar(a)
  r = ccall((:nmod_mat_inv, libflint), Int, (Ref{fpMatrix}, Ref{fpMatrix}), z, a)
...
function Solve._can_solve_internal_no_check(A::fpMatrix, b::fpMatrix, task::Symbol; side::Symbol=:left)
  ...
  fl = ccall((:nmod_mat_can_solve, libflint), Cint, (Ref{fpMatrix}, Ref{fpMatrix}, Ref{fpMatrix}), x, A, b)
...
function _solve_tril_right_flint!(x::fpMatrix, L::fpMatrix, B::fpMatrix, unit::Bool)
  ccall((:nmod_mat_solve_tril, libflint), Nothing, (Ref{fpMatrix}, Ref{fpMatrix}, Ref{fpMatrix}, Cint), x, L, B, Cint(unit))
  return nothing
end

function _solve_triu_right_flint!(x::fpMatrix, U::fpMatrix, B::fpMatrix, unit::Bool)
  ccall((:nmod_mat_solve_triu, libflint), Nothing, (Ref{fpMatrix}, Ref{fpMatrix}, Ref{fpMatrix}, Cint), x, U, B, Cint(unit))
  return nothing
end
...
function nullspace(M::fpMatrix)
  ...
  nullity = ccall((:nmod_mat_nullspace, libflint), Int, (Ref{fpMatrix}, Ref{fpMatrix}), N, M)
...
function lu!(P::Generic.Perm, x::fpMatrix)
  ...
  rank = ccall((:nmod_mat_lu, libflint), Int, (Ptr{Int}, Ref{fpMatrix}, Cint), P.d, x, Cint(false))
