...
==(x::AbsPowerSeriesRingElem, y::ZZRingElem) = precision(x) == 0 || ((length(x) == 0 && iszero(y)) || (length(x) == 1 && coeff(x, 0) == y))
...
==(x::RelPowerSeriesRingElem, y::ZZRingElem) = precision(x) == 0 || ((pol_length(x) == 0 && iszero(y)) || (pol_length(x) == 1 && valuation(x) == 0 && polcoeff(x, 0) == y))
...
==(x::PolyRingElem, y::ZZRingElem) = ((length(x) == 0 && iszero(y)) || (length(x) == 1 && coeff(x, 0) == y))
...
==(x::PolyRingElem{ZZRingElem}, y::ZZRingElem) = ((length(x) == 0 && iszero(y)) || (length(x) == 1 && coeff(x, 0) == y))
...
function ==(a::Generic.SparsePoly{T}, b::ZZRingElem) where {T<:RingElem}
  return length(a) == 0 ? iszero(b) : a.length == 1 & a.exps[1] == 0 && a.coeffs[1] == b
end
