...
function inv(x::zzModRingElem)
  ...
  #s = [UInt(0)]
  s = Ref{UInt}()
  g = ccall((:n_gcdinv, libflint), UInt, (Ptr{UInt}, UInt, UInt), s, x.data, R.n)
...
function divides(a::zzModRingElem, b::zzModRingElem)
  ...
  b1 = ccall((:n_invmod, libflint), UInt, (UInt, UInt), ub, divexact(m, gb))
  rr = R(q) * b1
  return true, rr
end
...
Random.Sampler(::Type{RNG}, R::zzModRing, n::Random.Repetition) where {RNG<:AbstractRNG} = Random.SamplerSimple(R, Random.Sampler(RNG, UInt(0):(R.n - 1), n))
