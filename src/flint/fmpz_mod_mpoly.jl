...
for (etype, rtype, ftype, ctype) in ((FpMPolyRingElem, FpMPolyRing, gfp_fmpz_mpoly_factor, FpFieldElem),)
  @eval begin
    ...
    function gens(R::($rtype))
      ...
      for i in 1:(R.nvars)
        z = R()
        ccall((:fmpz_mod_mpoly_gen, libflint), Nothing, (Ref{($etype)}, Int, Ref{($rtype)}), z, i - 1, R)
        A[i] = z
      end
      return A
    end
    ...
    function is_gen(a::($etype), i::Int)
      n = nvars(parent(a))
      (i <= 0 || i > n) && error("Index must be between 1 and $n")
      return Bool(ccall((:fmpz_mod_mpoly_is_gen, libflint), Cint, (Ref{($etype)}, Int, Ref{($rtype)}), a, i - 1, parent(a)))
    end

    function is_gen(a::($etype))
      return Bool(ccall((:fmpz_mod_mpoly_is_gen, libflint), Cint, (Ref{($etype)}, Int, Ref{($rtype)}), a, -1, parent(a)))
    end

    function deepcopy_internal(a::($etype), dict::IdDict)
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_set, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), z, a, parent(a))
      return z
    end
    ...
    function one(R::($rtype))
      z = R()
      ccall((:fmpz_mod_mpoly_one, libflint), Nothing, (Ref{($etype)}, Ref{($rtype)}), z, R)
      return z
    end

    function zero(R::($rtype))
      z = R()
      ccall((:fmpz_mod_mpoly_zero, libflint), Nothing, (Ref{($etype)}, Ref{($rtype)}), z, R)
      return z
    end

    function isone(a::($etype))
      return Bool(ccall((:fmpz_mod_mpoly_is_one, libflint), Cint, (Ref{($etype)}, Ref{($rtype)}), a, parent(a)))
    end

    function iszero(a::($etype))
      return Bool(ccall((:fmpz_mod_mpoly_is_zero, libflint), Cint, (Ref{($etype)}, Ref{($rtype)}), a, parent(a)))
    end
    ...
    function is_constant(a::($etype))
      return Bool(ccall((:fmpz_mod_mpoly_is_fmpz, libflint), Cint, (Ref{($etype)}, Ref{($rtype)}), a, parent(a)))
    end

    # TODO move this
    function expressify(a::($rtype); context=nothing)
      return Expr(:sequence, Expr(:text, "Multivariate Polynomial Ring in "), Expr(:series, symbols(a)...), Expr(:text, " over "), expressify(coefficient_ring(a); context=context))
    end
    ...
    function coeff(a::($etype), i::Int)
      ...
      z = ZZRingElem()
      ccall((:fmpz_mod_mpoly_get_term_coeff_fmpz, libflint), UInt, (Ref{ZZRingElem}, Ref{($etype)}, Int, Ref{($rtype)}), z, a, i - 1, parent(a))
    ...
    function coeff(a::($etype), b::($etype))
      ...
      z = ZZRingElem()
      ccall((:fmpz_mod_mpoly_get_coeff_fmpz_monomial, libflint), UInt, (Ref{ZZRingElem}, Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), z, a, b, parent(a))
      return base_ring(parent(a))(z)
    end
    ...
    function degree(a::($etype), i::Int)
      ...
      if degrees_fit_int(a)
        d = ccall((:fmpz_mod_mpoly_degree_si, libflint), Int, (Ref{($etype)}, Int, Ref{($rtype)}), a, i - 1, parent(a))
        return d
      else
    ...
    function degree_fmpz(a::($etype), i::Int)
      ...
      (i <= 0 || i > n) && error("Index must be between 1 and $n")
      d = ZZRingElem()
      ccall((:fmpz_mod_mpoly_degree_fmpz, libflint), Nothing, (Ref{ZZRingElem}, Ref{($etype)}, Int, Ref{($rtype)}), d, a, i - 1, parent(a))
      return d
    end

    # Return true if degrees fit into an Int
    function degrees_fit_int(a::($etype))
      return Bool(ccall((:fmpz_mod_mpoly_degrees_fit_si, libflint), Cint, (Ref{($etype)}, Ref{($rtype)}), a, parent(a)))
    end
    ...
    function degrees(a::($etype))
      ...
      ccall((:fmpz_mod_mpoly_degrees_si, libflint), Nothing, (Ptr{Int}, Ref{($etype)}, Ref{($rtype)}), degs, a, parent(a))
      return degs
    end
    ...
    function degrees_fmpz(a::($etype))
      ...
      ccall((:fmpz_mod_mpoly_degrees_fmpz, libflint), Nothing, (Ptr{Ref{ZZRingElem}}, Ref{($etype)}, Ref{($rtype)}), degs, a, parent(a))
      return degs
    end

    # Return true if degree fits into an Int
    function total_degree_fits_int(a::($etype))
      return Bool(ccall((:fmpz_mod_mpoly_total_degree_fits_si, libflint), Cint, (Ref{($etype)}, Ref{($rtype)}), a, parent(a)))
    end
    ...
    function total_degree(a::($etype))
      ...
      d = ccall((:fmpz_mod_mpoly_total_degree_si, libflint), Int, (Ref{($etype)}, Ref{($rtype)}), a, a.parent)
      return d
    end
    ...
    function total_degree_fmpz(a::($etype))
      d = ZZRingElem()
      ccall((:fmpz_mod_mpoly_total_degree_fmpz, libflint), Nothing, (Ref{ZZRingElem}, Ref{($etype)}, Ref{($rtype)}), d, a, parent(a))
      return d
    end
    ...
    function coeff(a::($etype), vars::Vector{Int}, exps::Vector{Int})
      ...
      ccall((:fmpz_mod_mpoly_get_coeff_vars_ui, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ptr{Int}, Ptr{Int}, Int, Ref{($rtype)}), z, a, vars, exps, length(vars), parent(a))
      return z
    end
    ...
    function -(a::($etype))
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_neg, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), z, a, parent(a))
      return z
    end
    ...
    function +(a::($etype), b::($etype))
      check_parent(a, b)
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_add, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), z, a, b, parent(a))
      return z
    end
    ...
    function -(a::($etype), b::($etype))
      check_parent(a, b)
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_sub, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), z, a, b, parent(a))
      return z
    end
    ...
    function *(a::($etype), b::($etype))
      check_parent(a, b)
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_mul, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), z, a, b, parent(a))
      return z
    end
    ...
    function +(a::($etype), b::UInt)
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_add_ui, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, UInt, Ref{($rtype)}), z, a, b, parent(a))
      return z
    end

    function +(a::($etype), b::Int)
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_add_si, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int, Ref{($rtype)}), z, a, b, parent(a))
      return z
    end

    function +(a::($etype), b::ZZRingElem)
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_add_fmpz, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{ZZRingElem}, Ref{($rtype)}), z, a, b, parent(a))
      return z
    end
    ...
    function -(a::($etype), b::UInt)
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_sub_ui, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, UInt, Ref{($rtype)}), z, a, b, parent(a))
      return z
    end

    function -(a::($etype), b::Int)
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_sub_si, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int, Ref{($rtype)}), z, a, b, parent(a))
      return z
    end

    function -(a::($etype), b::ZZRingElem)
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_sub_fmpz, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{ZZRingElem}, Ref{($rtype)}), z, a, b, parent(a))
      return z
    end
    ...
    function -(b::Integer, a::($etype))
      z = a - b
      ccall((:fmpz_mod_mpoly_neg, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), z, z, parent(a))
      return z
    end

    function *(a::($etype), b::UInt)
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_scalar_mul_ui, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, UInt, Ref{($rtype)}), z, a, b, parent(a))
      return z
    end

    function *(a::($etype), b::Int)
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_scalar_mul_si, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int, Ref{($rtype)}), z, a, b, parent(a))
      return z
    end

    function *(a::($etype), b::ZZRingElem)
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_scalar_mul_fmpz, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{ZZRingElem}, Ref{($rtype)}), z, a, b, parent(a))
      return z
    end
    ...
    *(b::Integer, a::($etype)) = a * base_ring(parent(a))(b)
    ...
    function ^(a::($etype), b::Int)
      ...
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_pow_ui, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, UInt, Ref{($rtype)}), z, a, UInt(b), parent(a))
      return z
    end
    ...
    function ^(a::($etype), b::ZZRingElem)
      b >= 0 || throw(DomainError(b, "Exponent must be non-negative"))
      z = parent(a)()
      ok = ccall((:fmpz_mod_mpoly_pow_fmpz, libflint), Cint, (Ref{($etype)}, Ref{($etype)}, Ref{ZZRingElem}, Ref{($rtype)}), z, a, b, parent(a))
    ...
    function gcd(a::($etype), b::($etype))
      check_parent(a, b)
      z = parent(a)()
      ok = ccall((:fmpz_mod_mpoly_gcd, libflint), Cint, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), z, a, b, a.parent)
    ...
    function (::Type{Fac{($etype)}})(fac::($ftype), preserve_input::Bool=true)
      ...
      for i in 0:(fac.num - 1)
        f = R()
        if preserve_input
          ccall((:fmpz_mod_mpoly_factor_get_base, libflint), Nothing, (Ref{($etype)}, Ref{($ftype)}, Int, Ref{($rtype)}), f, fac, i, R)
        else
          ccall((:fmpz_mod_mpoly_factor_swap_base, libflint), Nothing, (Ref{($etype)}, Ref{($ftype)}, Int, Ref{($rtype)}), f, fac, i, R)
        end
        F.fac[f] = ccall((:fmpz_mod_mpoly_factor_get_exp_si, libflint), Int, (Ref{($ftype)}, Int, Ref{($rtype)}), fac, i, R)
      end
      c = ZZRingElem()
      ccall((:fmpz_mod_mpoly_factor_get_constant_fmpz, libflint), UInt, (Ref{ZZRingElem}, Ref{($ftype)}), c, fac)
      F.unit = R(c)
      return F
    end
    ...
    function factor(a::($etype))
      ...
      R = parent(a)
      fac = ($ftype)(R)
      ok = ccall((:fmpz_mod_mpoly_factor, libflint), Cint, (Ref{($ftype)}, Ref{($etype)}, Ref{($rtype)}), fac, a, R)
    ...
    function factor_squarefree(a::($etype))
      ...
      R = parent(a)
      fac = ($ftype)(R)
      ok = ccall((:fmpz_mod_mpoly_factor_squarefree, libflint), Cint, (Ref{($ftype)}, Ref{($etype)}, Ref{($rtype)}), fac, a, R)
      iszero(ok) && error("unable to compute factorization")
      return Fac{($etype)}(fac, false)
    end
    ...
    function is_square(a::($etype))
      return Bool(ccall((:fmpz_mod_mpoly_is_square, libflint), Cint, (Ref{($etype)}, Ref{($rtype)}), a, a.parent))
    end

    function is_square_with_sqrt(a::($etype))
      q = parent(a)()
      flag = ccall((:fmpz_mod_mpoly_sqrt, libflint), Cint, (Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), q, a, a.parent)
      return (Bool(flag), q)
    end
    ...
    function ==(a::($etype), b::($etype))
      check_parent(a, b)
      return Bool(ccall((:fmpz_mod_mpoly_equal, libflint), Cint, (Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), a, b, a.parent))
    end

    function Base.isless(a::($etype), b::($etype))
      (!is_monomial(a) || !is_monomial(b)) && error("Not monomials in comparison")
      return ccall((:fmpz_mod_mpoly_cmp, libflint), Cint, (Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), a, b, a.parent) < 0
    end
    ...
    function ==(a::($etype), b::($ctype))
      return Bool(ccall((:fmpz_mod_mpoly_equal_fmpz, libflint), Cint, (Ref{($etype)}, Ref{ZZRingElem}, Ref{($rtype)}), a, data(b), a.parent))
    end

    ==(a::($ctype), b::($etype)) = b == a

    function ==(a::($etype), b::UInt)
      return Bool(ccall((:fmpz_mod_mpoly_equal_ui, libflint), Cint, (Ref{($etype)}, UInt, Ref{($rtype)}), a, b, parent(a)))
    end

    ==(a::UInt, b::($etype)) = b == a

    function ==(a::($etype), b::Int)
      return Bool(ccall((:fmpz_mod_mpoly_equal_si, libflint), Cint, (Ref{($etype)}, Int, Ref{($rtype)}), a, b, parent(a)))
    end
    ...
    function divides(a::($etype), b::($etype))
      ...
      z = parent(a)()
      d = ccall((:fmpz_mod_mpoly_divides, libflint), Cint, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), z, a, b, parent(a))
      return isone(d), z
    end
    ...
    function Base.div(a::($etype), b::($etype))
      check_parent(a, b)
      q = parent(a)()
      ccall((:fmpz_mod_mpoly_div, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), q, a, b, parent(a))
      return q
    end
    ...
    function Base.divrem(a::($etype), b::($etype))
      ...
      q = parent(a)()
      r = parent(a)()
      ccall((:fmpz_mod_mpoly_divrem, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), q, r, a, b, parent(a))
      return q, r
    end
    ...
    function Base.divrem(a::($etype), b::Vector{($etype)})
      ...
      r = parent(a)()
      ccall((:fmpz_mod_mpoly_divrem_ideal, libflint), Nothing, (Ptr{Ref{($etype)}}, Ref{($etype)}, Ref{($etype)}, Ptr{Ref{($etype)}}, Int, Ref{($rtype)}), q, r, a, b, len, parent(a))
      return q, r
    end
    ...
    function derivative(a::($etype), i::Int)
      ...
      (i <= 0 || i > n) && error("Index must be between 1 and $n")
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_derivative, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int, Ref{($rtype)}), z, a, i - 1, parent(a))
      return z
    end
    ...
    function zero!(a::($etype))
      ccall((:fmpz_mod_mpoly_zero, libflint), Nothing, (Ref{($etype)}, Ref{($rtype)}), a, parent(a))
      return a
    end

    function add!(a::($etype), b::($etype), c::($etype))
      ccall((:fmpz_mod_mpoly_add, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), a, b, c, parent(a))
      return a
    end

    function addeq!(a::($etype), b::($etype))
      ccall((:fmpz_mod_mpoly_add, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), a, a, b, parent(a))
      return a
    end

    function mul!(a::($etype), b::($etype), c::($etype))
      ccall((:fmpz_mod_mpoly_mul, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Ref{($etype)}, Ref{($rtype)}), a, b, c, parent(a))
      return a
    end
    ...
    function setcoeff!(a::($etype), n::Int, c::($ctype))
      if n > length(a)
        ccall((:fmpz_mod_mpoly_resize, libflint), Nothing, (Ref{($etype)}, Int, Ref{($rtype)}), a, n, a.parent)
      end
      ccall((:fmpz_mod_mpoly_set_term_coeff_fmpz, libflint), Nothing, (Ref{($etype)}, Int, Ref{ZZRingElem}, Ref{($rtype)}), a, n - 1, data(c), a.parent)
      return a
    end
    ...
    function combine_like_terms!(a::($etype))
      ccall((:fmpz_mod_mpoly_combine_like_terms, libflint), Nothing, (Ref{($etype)}, Ref{($rtype)}), a, a.parent)
      return a
    end
    ...
    function exponent_vector_fits(::Type{Int}, a::($etype), i::Int)
      b = ccall((:fmpz_mod_mpoly_term_exp_fits_si, libflint), Cint, (Ref{($etype)}, Int, Ref{($rtype)}), a, i - 1, parent(a))
      return Bool(b)
    end

    function exponent_vector_fits(::Type{UInt}, a::($etype), i::Int)
      b = ccall((:fmpz_mod_mpoly_term_exp_fits_ui, libflint), Cint, (Ref{($etype)}, Int, Ref{($rtype)}), a, i - 1, parent(a))
      return Bool(b)
    end

    function exponent_vector!(z::Vector{Int}, a::($etype), i::Int)
      ccall((:fmpz_mod_mpoly_get_term_exp_si, libflint), Nothing, (Ptr{Int}, Ref{($etype)}, Int, Ref{($rtype)}), z, a, i - 1, parent(a))
      return z
    end

    function exponent_vector!(z::Vector{UInt}, a::($etype), i::Int)
      ccall((:fmpz_mod_mpoly_get_term_exp_ui, libflint), Nothing, (Ptr{UInt}, Ref{($etype)}, Int, Ref{($rtype)}), z, a, i - 1, parent(a))
      return z
    end

    function exponent_vector!(z::Vector{ZZRingElem}, a::($etype), i::Int)
      ccall((:fmpz_mod_mpoly_get_term_exp_fmpz, libflint), Nothing, (Ptr{Ref{ZZRingElem}}, Ref{($etype)}, Int, Ref{($rtype)}), z, a, i - 1, parent(a))
      return z
    end
    ...
    function set_exponent_vector!(a::($etype), n::Int, exps::Vector{UInt})
      ...
      ccall((:fmpz_mod_mpoly_set_term_exp_ui, libflint), Nothing, (Ref{($etype)}, Int, Ptr{UInt}, Ref{($rtype)}), a, n - 1, exps, parent(a))
      return a
    end
    ...
    function set_exponent_vector!(a::($etype), n::Int, exps::Vector{Int})
      if n > length(a)
        ccall((:fmpz_mod_mpoly_resize, libflint), Nothing, (Ref{($etype)}, Int, Ref{($rtype)}), a, n, parent(a))
      end
      ccall((:fmpz_mod_mpoly_set_term_exp_ui, libflint), Nothing, (Ref{($etype)}, Int, Ptr{Int}, Ref{($rtype)}), a, n - 1, exps, parent(a))
      return a
    end
    ...
    function set_exponent_vector!(a::($etype), n::Int, exps::Vector{ZZRingElem})
      ...
      ccall((:fmpz_mod_mpoly_set_term_exp_fmpz, libflint), Nothing, (Ref{($etype)}, Int, Ptr{ZZRingElem}, Ref{($rtype)}), a, n - 1, exps, parent(a))
      return a
    end
    ...
    function exponent(a::($etype), i::Int, j::Int)
      ...
      return ccall((:fmpz_mod_mpoly_get_term_var_exp_ui, libflint), Int, (Ref{($etype)}, Int, Int, Ref{($rtype)}), a, i - 1, j - 1, parent(a))
    end
    ...
    function coeff(a::($etype), exps::Vector{UInt})
      z = ZZRingElem()
      ccall((:fmpz_mod_mpoly_get_coeff_fmpz_ui, libflint), UInt, (Ref{ZZRingElem}, Ref{($etype)}, Ptr{UInt}, Ref{($rtype)}), z, a, exps, parent(a))
      return base_ring(parent(a))(z)
    end
    ...
    function coeff(a::($etype), exps::Vector{Int})
      z = ZZRingElem()
      ccall((:fmpz_mod_mpoly_get_coeff_fmpz_ui, libflint), UInt, (Ref{ZZRingElem}, Ref{($etype)}, Ptr{Int}, Ref{($rtype)}), z, a, exps, parent(a))
      return base_ring(parent(a))(z)
    end
    ...
    function setcoeff!(a::($etype), exps::Vector{UInt}, b::($ctype))
      ccall((:fmpz_mod_mpoly_set_coeff_fmpz_ui, libflint), Nothing, (Ref{($etype)}, Ref{ZZRingElem}, Ptr{UInt}, Ref{($rtype)}), a, data(b), exps, parent(a))
      return a
    end
    ...
    function setcoeff!(a::($etype), exps::Vector{Int}, b::($ctype))
      ccall((:fmpz_mod_mpoly_set_coeff_fmpz_ui, libflint), Nothing, (Ref{($etype)}, Ref{ZZRingElem}, Ptr{Int}, Ref{($rtype)}), a, data(b), exps, parent(a))
      return a
    end

    # Set the coefficient of the term with the given exponent vector to the
    # given integer. Removal of a zero term is performed.
    setcoeff!(a::($etype), exps::Vector{Int}, b::Integer) = setcoeff!(a, exps, base_ring(parent(a))(b))
    ...
    function sort_terms!(a::($etype))
      ccall((:fmpz_mod_mpoly_sort_terms, libflint), Nothing, (Ref{($etype)}, Ref{($rtype)}), a, parent(a))
      return a
    end
    ...
    function term(a::($etype), i::Int)
      z = parent(a)()
      ccall((:fmpz_mod_mpoly_get_term, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int, Ref{($rtype)}), z, a, i - 1, parent(a))
      return z
    end

    # Sets the given polynomial m to the i-th monomial of the polynomial
    function monomial!(m::($etype), a::($etype), i::Int)
      ccall((:fmpz_mod_mpoly_get_term_monomial, libflint), Nothing, (Ref{($etype)}, Ref{($etype)}, Int, Ref{($rtype)}), m, a, i - 1, a.parent)
      return m
    end
