...
function _reduce(a::ZZRingElem, ctx::fmpz_mod_ctx_struct)
  b = ZZRingElem()
  ccall((:fmpz_mod_set_fmpz, libflint), Nothing, (Ref{ZZRingElem}, Ref{ZZRingElem}, Ref{fmpz_mod_ctx_struct}), b, a, ctx)
  return b
end
...
function divides(a::ZZModRingElem, b::ZZModRingElem)
  ...
  ub = divexact(B, gb)
  b1 = ZZRingElem()
  ccall((:fmpz_invmod, libflint), Nothing, (Ref{ZZRingElem}, Ref{ZZRingElem}, Ref{ZZRingElem}), b1, ub, divexact(m, gb))
  rr = R(q) * b1
  return true, rr
end
...
Random.Sampler(::Type{RNG}, R::ZZModRing, n::Random.Repetition) where {RNG<:AbstractRNG} = Random.SamplerSimple(R, Random.Sampler(RNG, BigInt(0):(BigInt(R.n) - 1), n))
...
rand(rng::AbstractRNG, sp::SamplerTrivial{<:Make2{ZZModRingElem,ZZModRing,<:AbstractArray{<:IntegerUnion}}}) = sp[][1](rand(rng, sp[][2]))
