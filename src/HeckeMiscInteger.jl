...
Base.in(x::IntegerUnion, r::AbstractRange{ZZRingElem}) = !isempty(r) && first(r) <= x <= last(r) && mod(convert(ZZRingElem, x), step(r)) == mod(first(r), step(r))
...
function euler_phi(x::Fac{ZZRingElem})
  return prod((p - 1) * p^(v - 1) for (p, v) in x.fac)
end
...
fmpz_unitrange_last(start::ZZRingElem, stop::ZZRingElem) = ifelse(stop >= start, stop, start - one(ZZRingElem))
...
function rand(rng::AbstractRNG, a::ZZRingElemUnitRange)
  ...
  while true
    s = ZZRingElem(0)
    for i in 1:nl
      ccall((:fmpz_mul_2exp, libflint), Nothing, (Ref{ZZRingElem}, Ref{ZZRingElem}, Int), s, s, c)
      ccall((:fmpz_add_ui, libflint), Nothing, (Ref{ZZRingElem}, Ref{ZZRingElem}, UInt), s, s, rand(rng, Base.GMP.Limb))
    end
    if high > 0
      s = s << high
