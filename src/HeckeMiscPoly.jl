...
function resultant(f::ZZPolyRingElem, g::ZZPolyRingElem, d::ZZRingElem, nb::Int)
  z = ZZRingElem()
  ccall((:fmpz_poly_resultant_modular_div, libflint), Nothing, (Ref{ZZRingElem}, Ref{ZZPolyRingElem}, Ref{ZZPolyRingElem}, Ref{ZZRingElem}, Int), z, f, g, d, nb)
  return z
end
...
function rem!(z::FqPolyRepPolyRingElem, x::FqPolyRepPolyRingElem, y::FqPolyRepPolyRingElem)
  ccall((:fq_poly_rem, libflint), Nothing, (Ref{FqPolyRepPolyRingElem}, Ref{FqPolyRepPolyRingElem}, Ref{FqPolyRepPolyRingElem}, Ref{FqPolyRepPolyRing}), z, x, y, parent(x))
  return z
end
...
function fmpz_poly_to_nmod_poly_raw!(r::zzModPolyRingElem, a::ZZPolyRingElem)
  ccall((:fmpz_poly_get_nmod_poly, libflint), Nothing, (Ref{zzModPolyRingElem}, Ref{ZZPolyRingElem}), r, a)
end

function fmpz_poly_to_gfp_poly_raw!(r::fpPolyRingElem, a::ZZPolyRingElem)
  ccall((:fmpz_poly_get_nmod_poly, libflint), Nothing, (Ref{fpPolyRingElem}, Ref{ZZPolyRingElem}), r, a)
end
...
function fmpz_poly_to_fmpz_mod_poly_raw!(r::ZZModPolyRingElem, a::ZZPolyRingElem)
  ccall((:fmpz_poly_get_fmpz_mod_poly, libflint), Nothing, (Ref{ZZModPolyRingElem}, Ref{ZZPolyRingElem}, Ref{fmpz_mod_ctx_struct}), r, a, r.parent.base_ring.ninv)
end
...
function (a::ZZPolyRing)(b::QQPolyRingElem)
  ...
  z = a()
  ccall((:fmpq_poly_get_numerator, libflint), Nothing, (Ref{ZZPolyRingElem}, Ref{QQPolyRingElem}), z, b)
  return z
end
...
function roots(f::T) where {T<:Union{fqPolyRepPolyRingElem,FqPolyRepPolyRingElem}} # should be in Nemo and
  ...
  f = gcd(f, x)
  l = factor(f).fac
  return elem_type(base_ring(f))[-divexact(constant_coefficient(x), leading_coefficient(x)) for x in keys(l) if degree(x) == 1]
end

function setcoeff!(z::fqPolyRepPolyRingElem, n::Int, x::ZZRingElem)
  ccall((:fq_nmod_poly_set_coeff_fmpz, libflint), Nothing, (Ref{fqPolyRepPolyRingElem}, Int, Ref{ZZRingElem}, Ref{fqPolyRepField}), z, n, x, base_ring(parent(z)))
  return z
end
...
function factor_squarefree(f::PolyRingElem{<:FieldElement})
  ...
  return Fac(unit(fac), Dict{typeof(f),Int}(facs[i] => es[i] for i in 1:length(es)))
end
...
function factor_equal_deg(x::fpPolyRingElem, d::Int)
  ...
  ccall((:nmod_poly_factor_equal_deg, libflint), UInt, (Ref{gfp_poly_factor}, Ref{fpPolyRingElem}, Int), fac, x, d)
...
function factor_equal_deg(x::FpPolyRingElem, d::Int)
  ...
  ccall((:fmpz_mod_poly_factor_equal_deg, libflint), UInt, (Ref{gfp_fmpz_poly_factor}, Ref{FpPolyRingElem}, Int, Ref{fmpz_mod_ctx_struct}), fac, x, d, x.parent.base_ring.ninv)
  res = Vector{FpPolyRingElem}(undef, fac.num)
  for i in 1:(fac.num)
    f = parent(x)()
    ccall((:fmpz_mod_poly_factor_get_fmpz_mod_poly, libflint), Nothing, (Ref{FpPolyRingElem}, Ref{gfp_fmpz_poly_factor}, Int, Ref{fmpz_mod_ctx_struct}), f, fac, i - 1, x.parent.base_ring.ninv)
    res[i] = f
  end
  return res
end
