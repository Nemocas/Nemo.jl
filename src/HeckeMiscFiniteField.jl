...
function Base.iterate(F::fqPolyRepField, st::Vector{UInt})
  ...
  d = F()
  ccall((:fq_nmod_init2, libflint), Nothing, (Ref{fqPolyRepFieldElem}, Ref{fqPolyRepField}), d, F)
  for j in 1:length(st)
    ccall((:nmod_poly_set_coeff_ui, libflint), Nothing, (Ref{fqPolyRepFieldElem}, Int, UInt), d, j - 1, st[j])
  end
...
function Base.iterate(F::FqPolyRepField, st::Vector{ZZRingElem})
  ...
  d = F()
  ccall((:fq_init2, libflint), Nothing, (Ref{FqPolyRepFieldElem}, Ref{FqPolyRepField}), d, F)
  for j in 1:length(st)
    #ccall((:fmpz_poly_set_coeff_fmpz, libflint), (Ref{ZZPolyRingElem}, Int, Ref{ZZRingElem}), g, j - 1, st[j])
    ccall((:fmpz_poly_set_coeff_fmpz, libflint), Nothing, (Ref{FqPolyRepFieldElem}, Int, Ref{ZZRingElem}), d, j - 1, st[j])
  end
...
function frobenius!(a::fqPolyRepFieldElem, b::fqPolyRepFieldElem, i::Int=1)
  ccall((:fq_nmod_frobenius, libflint), Nothing, (Ref{fqPolyRepFieldElem}, Ref{fqPolyRepFieldElem}, Int, Ref{fqPolyRepField}), a, b, i, a.parent)
end
