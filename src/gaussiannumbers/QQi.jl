...
function (a::QQiField)(b::Union{Integer,ZZRingElem,Rational,QQFieldElem}, c::Union{Integer,ZZRingElem,Rational,QQFieldElem})
...
function Base.convert(::Type{Complex{Rational{T}}}, a::QQiFieldElem) where {T<:Integer}
  return Complex{Rational{T}}(Base.convert(Rational{T}, real(a)), Base.convert(Rational{T}, imag(a)))
end
...
function reduce!(z::QQiFieldElem)
  g = ZZRingElem()
  ccall((:fmpz_gcd3, libflint), Nothing, (Ref{ZZRingElem}, Ref{ZZRingElem}, Ref{ZZRingElem}, Ref{ZZRingElem}), g, z.num.x, z.den, z.num.y)
...
for (As, Bs) in [
  ...
  [(ZZiRingElem, QQiFieldElem), (Integer, Rational, ZZRingElem, QQFieldElem, Complex{<:Integer}, Complex{<:Rational}, ZZiRingElem, QQiFieldElem)],
]
  for A in As, B in Bs
    @eval begin
      function //(a::($A), b::($B))
