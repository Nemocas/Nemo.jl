...
function copy_data(a::ZZRingElem)
  z = Ref(0)
  ccall((:fmpz_init_set, libflint), Nothing, (Ref{Int}, Ref{ZZRingElem}), z, a)
  return z[]
end

function shift_right!(z::ZZRingElem, a::ZZRingElem, b::Union{Int,UInt})
  ccall((:fmpz_fdiv_q_2exp, libflint), Nothing, (Ref{ZZRingElem}, Ref{ZZRingElem}, UInt), z, a, UInt(b))
  return z
end
...
function _shortest_l_infinity(c::ZZRingElem, b::ZZRingElem, a::ZZRingElem)
  ...
  x = _fmpq_ball(copy_data(a), steal_data!(b_plus_c), copy_data(a), steal_data!(b_minus_c), 0)
  ccall((:_fmpq_ball_get_cfrac, libflint), Nothing, (Ref{_fmpq_cfrac_list}, Ref{_fmpz_mat22}, Cint, Ref{_fmpq_ball}), s, m, 1, x)
  ...
  v12 = add!(xld, xld, xrd)
  shift_right!(v12, v12, 1)
  v22 = add!(xln, xln, xrn)
  shift_right!(v22, v22, 1)
  if m.det < 0
    neg!(v12, v12)
  else
    neg!(v22, v22)
...
function _doit_exact!(xn::ZZRingElem, xd::ZZRingElem, v::Vector{ZZRingElem}, lim::Int, wantM::Bool)
  ...
  if ok
    ...
    ccall((:_fmpq_ball_get_cfrac, libflint), Nothing, (Ref{_fmpq_cfrac_list}, Ref{_fmpz_mat22}, Cint, Ref{_fmpq_ball}), s, m, wantM, x)
    _push_and_clear!(v, s)
  end
  xn.d = x.left_num
  x.left_num = 0
  xd.d = x.left_den
  x.left_den = 0
  return (steal_fmpz_data(m._11), steal_fmpz_data(m._12), steal_fmpz_data(m._21), steal_fmpz_data(m._22))
end
...
function _doit_ball!(xln::ZZRingElem, xld::ZZRingElem, xrn::ZZRingElem, xrd::ZZRingElem, v::Vector{ZZRingElem}, lim::Int, wantM::Bool)
  x = _fmpq_ball(steal_data!(xln), steal_data!(xld), steal_data!(xrn), steal_data!(xrd), 0)
  m = _fmpz_mat22(1, 0, 0, 1, 1)
  s = _fmpq_cfrac_list(C_NULL, 0, 0, lim, 0, 0)
  ccall((:_fmpq_ball_get_cfrac, libflint), Nothing, (Ref{_fmpq_cfrac_list}, Ref{_fmpz_mat22}, Cint, Ref{_fmpq_ball}), s, m, wantM, x)
  _push_and_clear!(v, s)
...
function _continued_fraction_ball(xln::ZZRingElem, xld::ZZRingElem, xrn::ZZRingElem, xrd::ZZRingElem, lim::Int, wantM::Bool)
  lim = lim < 1 ? typemax(Int) : lim
  v = ZZRingElem[]
  if xln > xld
...
function _left_and_right(x::ArbFieldElem)
  ...
  b = ZZRingElem()
  f = ZZRingElem()
  ccall((:arb_get_interval_fmpz_2exp, libflint), Nothing, (Ref{ZZRingElem}, Ref{ZZRingElem}, Ref{ZZRingElem}, Ref{ArbFieldElem}), a, b, f, x)
...
function Base.getindex(it::ConvergentsIterator{T,R}, n) where {T,R}
  m = mapreduce(q -> (q, T(1), T(1), T(0)), (x, y) -> (x[1] * y[1] + x[2] * y[3], x[1] * y[2] + x[2] * y[4], x[3] * y[1] + x[4] * y[3], x[3] * y[2] + x[4] * y[4]), it.v[1:n])
  return m[1]//m[3]
end
...
function Base.iterate(it::ConvergentsIterator{T,R}, state) where {T,R}
  ...
  m = state[1]
  addmul!(m[2], m[1], it.v[n])
  swap!(m[2], m[1])
  addmul!(m[4], m[3], it.v[n])
  swap!(m[4], m[3])
