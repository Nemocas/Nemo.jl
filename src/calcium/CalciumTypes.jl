...
@doc qq_field_doc struct QQBarField <: Field end

const CalciumQQBar = QQBarField()

@doc qq_field_doc mutable struct QQBarFieldElem <: FieldElem
  coeffs::Ptr{Nothing}
  alloc::Int
  length::Int
  real_mid_exp::Int     # ZZRingElem
...
function options(C::CalciumField)
  d = Dict{Symbol,Int}()
  for i in 1:length(ca_ctx_options)
...
mutable struct CalciumFieldElem <: FieldElem
  ...
  function CalciumFieldElem(ctx::CalciumField)
    z = new()
    ccall((:ca_init, libflint), Nothing, (Ref{CalciumFieldElem}, Ref{CalciumField}), z, ctx)
    z.parent = ctx
    z.parent.refcount += 1
...
function _ca_clear_fn(a::CalciumFieldElem)
  ccall((:ca_clear, libflint), Nothing, (Ref{CalciumFieldElem}, Ref{CalciumField}), a, a.parent)
