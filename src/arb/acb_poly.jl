...
length(x::acb_poly) = ccall((:acb_poly_length, libflint), Int,
                                   (Ref{acb_poly},), x)
...
function isreal(x::acb_poly)
  return ccall((:acb_poly_is_real, libflint), Cint, (Ref{acb_poly}, ), x) != 0
end
...
function -(x::acb_poly)
  z = parent(x)()
  ccall((:acb_poly_neg, libflint), Nothing, (Ref{acb_poly}, Ref{acb_poly}), z, x)
  return z
end
...
function acb_vec(b::Vector{acb})
   ...
   for i=1:length(b)
       ccall((:acb_set, libflint), Nothing, (Ptr{acb_struct}, Ref{acb}),
...
function array(R::AcbField, v::Ptr{acb_struct}, n::Int)
   ...
   for i=1:n
       r[i] = R()
       ccall((:acb_set, libflint), Nothing, (Ref{acb}, Ptr{acb_struct}),
            ...
            if target > 0
                for i = 0 : deg-1
                    ...
                    im = ccall((:acb_imag_ptr, libflint), Ptr{arb_struct},
                        (Ptr{acb}, ), roots + i * sizeof(acb_struct))
                    t = ccall((:arb_rad_ptr, libflint), Ptr{mag_struct}, (Ptr{arb}, ), re)
                    u = ccall((:arb_rad_ptr, libflint), Ptr{mag_struct}, (Ptr{arb}, ), im)
...
function zero!(z::acb_poly)
   ccall((:acb_poly_zero, libflint), Nothing, (Ref{acb_poly},), z)
   return z
end
