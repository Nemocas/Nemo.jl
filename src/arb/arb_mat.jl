...
base_ring(a::ArbMatSpace) = ArbField()

base_ring(a::arb_mat) = ArbField()
...
function inv(x::arb_mat)
  ...
  r = ccall((:arb_mat_inv, libarb), Cint,
              (Ref{arb_mat}, Ref{arb_mat}, Int), z, x, precision(Balls))
...
function charpoly(x::ArbPolyRing, y::arb_mat)
  ...
  ccall((:arb_mat_charpoly, libarb), Nothing,
              (Ref{arb_poly}, Ref{arb_mat}, Int), z, y, precision(Balls))
  return z
end
...
function det(x::arb_mat)
  ...
  ccall((:arb_mat_det, libarb), Nothing,
              (Ref{arb}, Ref{arb_mat}, Int), z, x, precision(Balls))
  return z
end
...
function (x::ArbMatSpace)(y::AbstractMatrix{T}) where {T <: Union{Int, UInt, fmpz, fmpq, Float64, BigFloat, arb, AbstractString}}
  ...
  z = arb_mat(nrows(x), ncols(x), y, precision(Balls))
  return z
end
...
function matrix(R::ArbField, arr::AbstractMatrix{T}) where {T <: Union{Int, UInt, fmpz, fmpq, Float64, BigFloat, arb, AbstractString}}
   z = arb_mat(size(arr, 1), size(arr, 2), arr, precision(Balls))
   return z
end
...
function matrix(R::ArbField, r::Int, c::Int, arr::AbstractVector{T}) where {T <: Union{Int, UInt, fmpz, fmpq, Float64, BigFloat, arb, AbstractString}}
   _check_dim(r, c, arr)
   z = arb_mat(r, c, arr, precision(Balls))
   return z
end
