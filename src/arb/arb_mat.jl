...
function getindex!(z::arb, x::arb_mat, r::Int, c::Int)
  GC.@preserve x begin
     v = ccall((:arb_mat_entry_ptr, libflint), Ptr{arb},
                 (Ref{arb_mat}, Int, Int), x, r - 1, c - 1)
     ccall((:arb_set, libflint), Nothing, (Ref{arb}, Ptr{arb}), z, v)
  end
  return z
end
...
for T in [Int, UInt, ZZRingElem, QQFieldElem, Float64, BigFloat, arb, AbstractString]
   @eval begin
      @inline function setindex!(x::arb_mat, y::$T, r::Int, c::Int)
         ...
         GC.@preserve x begin
            z = ccall((:arb_mat_entry_ptr, libflint), Ptr{arb},
                      (Ref{arb_mat}, Int, Int), x, r - 1, c - 1)
...
function one(x::ArbMatSpace)
  z = x()
  ccall((:arb_mat_one, libflint), Nothing, (Ref{arb_mat}, ), z)
  return z
end
...
function deepcopy_internal(x::arb_mat, dict::IdDict)
  ...
  ccall((:arb_mat_set, libflint), Nothing, (Ref{arb_mat}, Ref{arb_mat}), z, x)
...
function -(x::arb_mat)
  z = similar(x)
  ccall((:arb_mat_neg, libflint), Nothing, (Ref{arb_mat}, Ref{arb_mat}), z, x)
  return z
end
...
function ==(x::arb_mat, y::arb_mat)
  ...
  !fl && return false
  r = ccall((:arb_mat_eq, libflint), Cint, (Ref{arb_mat}, Ref{arb_mat}), x, y)
  return Bool(r)
end

function !=(x::arb_mat, y::arb_mat)
  r = ccall((:arb_mat_ne, libflint), Cint, (Ref{arb_mat}, Ref{arb_mat}), x, y)
  return Bool(r)
end
...
function cholesky(x::arb_mat)
  ...
  fl = ccall((:arb_mat_cho, libflint), Cint, (Ref{arb_mat}, Ref{arb_mat}, Int), z, x, p)
