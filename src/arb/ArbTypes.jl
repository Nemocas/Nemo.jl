...
mutable struct arb <: FieldElem
  ...
  function arb()
    z = new()
    ccall((:arb_init, libflint), Nothing, (Ref{arb}, ), z)
    finalizer(_arb_clear_fn, z)
    return z
  end
  ...
  function arb(mid::arb, rad::arb)
    z = new()
    ccall((:arb_init, libflint), Nothing, (Ref{arb}, ), z)
    ccall((:arb_set, libflint), Nothing, (Ref{arb}, Ref{arb}), z, mid)
    ccall((:arb_add_error, libflint), Nothing, (Ref{arb}, Ref{arb}), z, rad)
    finalizer(_arb_clear_fn, z)
    return z
  end
  ...
  #  z = new()
  #  ccall((:arb_init, libflint), Nothing, (Ref{arb}, ), z)
  #  ccall((:arb_set_arf, libflint), Nothing, (Ref{arb}, Ptr{arf}), z, x)
  #  finalizer(_arb_clear_fn, z)
  #  return z
  #end
end
...
function _arb_clear_fn(x::arb)
  ccall((:arb_clear, libflint), Nothing, (Ref{arb}, ), x)
end
...
mutable struct acb <: FieldElem
  ...
  function acb()
    z = new()
    ccall((:acb_init, libflint), Nothing, (Ref{acb}, ), z)
    finalizer(_acb_clear_fn, z)
    return z
  end
...
function _acb_clear_fn(x::acb)
  ccall((:acb_clear, libflint), Nothing, (Ref{acb}, ), x)
end
...
mutable struct arb_poly <: PolyRingElem{arb}
  ...
  function arb_poly()
    z = new()
    ccall((:arb_poly_init, libflint), Nothing, (Ref{arb_poly}, ), z)
  ...
  function arb_poly(x::arb_poly)
    z = new() 
    ccall((:arb_poly_init, libflint), Nothing, (Ref{arb_poly}, ), z)
    ccall((:arb_poly_set, libflint), Nothing, (Ref{arb_poly}, Ref{arb_poly}), z, x)
    finalizer(_arb_poly_clear_fn, z)
    return z
  end
...
function _arb_poly_clear_fn(x::arb_poly)
  ccall((:arb_poly_clear, libflint), Nothing, (Ref{arb_poly}, ), x)
end
...
mutable struct acb_poly <: PolyRingElem{acb}
  ...
  function acb_poly()
    z = new()
    ccall((:acb_poly_init, libflint), Nothing, (Ref{acb_poly}, ), z)
    finalizer(_acb_poly_clear_fn, z)
    return z
  end
  ...
  function acb_poly(x::acb_poly)
    z = new() 
    ccall((:acb_poly_init, libflint), Nothing, (Ref{acb_poly}, ), z)
    ccall((:acb_poly_set, libflint), Nothing, (Ref{acb_poly}, Ref{acb_poly}), z, x)
    finalizer(_acb_poly_clear_fn, z)
    return z
  end
...
function _acb_poly_clear_fn(x::acb_poly)
  ccall((:acb_poly_clear, libflint), Nothing, (Ref{acb_poly}, ), x)
end
...
mutable struct arb_mat <: MatElem{arb}
  ...
  function arb_mat(r::Int, c::Int)
    z = new()
    ccall((:arb_mat_init, libflint), Nothing, (Ref{arb_mat}, Int, Int), z, r, c)
    finalizer(_arb_mat_clear_fn, z)
    return z
  end
  ...
  function arb_mat(r::Int, c::Int, arr::AbstractMatrix{T}) where {T <: Union{Int, UInt, ZZRingElem, Float64, BigFloat, arb}}
    ...
    GC.@preserve z for i = 1:r
      for j = 1:c
        el = ccall((:arb_mat_entry_ptr, libflint), Ptr{arb},
...
function _arb_mat_clear_fn(x::arb_mat)
  ccall((:arb_mat_clear, libflint), Nothing, (Ref{arb_mat}, ), x)
end
...
mutable struct ComplexMat <: MatElem{ComplexFieldElem}
  ...
  function ComplexMat(r::Int, c::Int, arr::AbstractMatrix{T}) where {T <: Union{Int, UInt, Float64, ZZRingElem}}
    ...
    GC.@preserve z for i = 1:r
      for j = 1:c
        el = ccall((:acb_mat_entry_ptr, libflint), Ptr{acb},
...
mutable struct acb_mat <: MatElem{acb}
  ...
  function acb_mat(r::Int, c::Int)
    z = new()
    ccall((:acb_mat_init, libflint), Nothing, (Ref{acb_mat}, Int, Int), z, r, c)
    finalizer(_acb_mat_clear_fn, z)
    return z
  end
...
function _acb_mat_clear_fn(x::acb_mat)
  ccall((:acb_mat_clear, libflint), Nothing, (Ref{acb_mat}, ), x)
end
