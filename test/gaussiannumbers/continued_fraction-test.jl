@testset "continued_fraction._shortest_l_infinity" begin
  for k in 1:1000
    ...
    for x1 in -10:10, x2 in -10:10
      @test iszero(t1 + x1) && iszero(t2 + x2) || max(abs((t1 + x1) * c), abs((t1 + x1) * b + (t2 + x2) * a)) >= m
    end
  end
end
...
@testset "continued_fraction.ArbFieldElem" begin
  set_precision!(Balls, 100) do
    ...
    @test continued_fraction_with_matrix(-1 / const_pi(CC); limit=4) == (ZZRingElem[-1, 1, 2, 7], matrix(ZZ, [-7 -1; 22 3]))
