...
@testset "ZZRingElem.rand" begin
  ...
  @testset "Nemo seeding" begin
    for seed in (rand(UInt128), rand(Int8(0):typemax(Int8)))
      Nemo.randseed!(seed)
      a = [rand_bits(ZZ, i) for i in 1:99] # must test for i > 64, to exercise
      # both Flint's RNGs
      Nemo.randseed!(seed)
      @test a == [rand_bits(ZZ, i) for i in 1:99]
    end
...
@testset "ZZRingElem.crt" begin
  ...
  for s in (true, false)
    ...
    for (r1, m1, r2, m2) in (
      ...
      @test crt(ZZ(r1), ZZ(m1), r2, m2, s) == crt(ZZ(r1), ZZ(m1), ZZ(r2), ZZ(m2), s)

      @test crt_with_lcm(ZZ(r1), ZZ(m1), r2, m2, s) == crt_with_lcm(ZZ(r1), ZZ(m1), ZZ(r2), ZZ(m2), s)
    end
  end
...
@testset "ZZRingElem.number_theoretic" begin
  ...
  @test factorial(ZZ(100)) ==
    ZZRingElem("93326215443944152681699238856266700490715968264381621468592963895217599993229915608941463976156518286253697920827223758251185210916864000000000000000000000000")
  ...
  for T in [Int, ZZRingElem]
    for iters in 1:1000
      ...
      @test m1 == -1 ||
        ...
        m2 == -1 ||
        n2 == -1 ||
        kronecker_symbol(m1 * m2, n1 * n2) == kronecker_symbol(m1, n1) * kronecker_symbol(m1, n2) * kronecker_symbol(m2, n1) * kronecker_symbol(m2, n2)
    end
