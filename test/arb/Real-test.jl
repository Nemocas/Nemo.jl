...
@testset "arf.hecke_semantics" begin
  ...
  y = ZZRingElem(3)^100
  ccall((:arf_set_fmpz, Nemo.libflint), Nothing, (Ref{Nemo.arf_struct}, Ref{ZZRingElem}), x, y)
  ccall((:arf_clear, Nemo.libflint), Nothing, (Ref{Nemo.arf_struct},), x)
end
...
@testset "RealFieldElem.rand" begin
  ...
  for _ in 1:n
    ...
    if midpoint(r_precise) != 0 != radius(r_precise)
      @test R(0.99) * R(2)^(-6 - precision(Balls)) < abs(radius(r_precise) / midpoint(r_precise)) < R(1.01) * R(2)^(3 - precision(Balls))
    end
